...
def _model_meta(model: dict) -> dict:
...
def _resolve_model_tool_ids(app, model_id: str) -> list[str]:
    ...
    tool_ids = _model_meta(model).get('toolIds', [])
...
async def _resolve_model_features(app, model_id: str) -> dict:
    ...
    + model.info.meta.capabilities). Enables features like image_generation
    when the model has them as defaults
...
def _resolve_model_filter_ids(app, model_id: str) -> list[str]:
    ...
    models = getattr(app.state, 'MODELS', {})
    model = models.get(model_id, {})
    filter_ids = _model_meta(model).get('defaultFilterIds', [])
...
def _resolve_model_terminal_id(app, model_id: str) -> Optional[str]:
    ...
    models = getattr(app.state, 'MODELS', {})
    model = models.get(model_id, {})
    return _model_meta(model).get('terminalId') or None
    ...
    try:
        async with aiohttp.ClientSession(timeout=aiohttp.ClientTimeout(total=10)) as session:
            async with session.post(
                ...
                if resp.status != 200:
                    await resp.read()
                    log.warning('Failed to set terminal CWD: HTTP %s', resp.status)
    except Exception as e:
        log.warning('Failed to set terminal CWD (%s)', type(e).__name__)
