...
async def post_webhook(name: str, url: str, message: str, event_data: dict, description: str | None = None) -> bool:
    try:
        log.debug('Posting webhook event=%s', event_data.get('event', 'unspecified'))
        ...
        log.debug('Sending webhook payload type=%s', type(payload).__name__)
        async with get_ssrf_safe_session() as session:
            async with session.post(
                ...
                await r.content.read(64 * 1024)
                r.raise_for_status()
                log.debug('Webhook delivered status=%s', r.status)

        return True
    except Exception as e:
        log.exception('Webhook delivery failed (%s)', type(e).__name__)
        return False
