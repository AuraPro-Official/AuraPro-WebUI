...
from open_webui.utils.files import (
    ...
    get_audio_base64_from_file_id,
    get_openai_audio_base64_from_data_url,
...
def ensure_metadata_dict(value):
    metadata = value if isinstance(value, dict) else {}
    for key in ('params', 'features', 'tools'):
...
def get_citation_source_from_tool_result(
    ...
    """
    _EXPECTS_LIST = {'query_knowledge_files'}
...
async def chat_completion_tools_handler(
    ...
    metadata = ensure_metadata_dict(extra_params.get('__metadata__'))
    ...
    try:
        ...
        log.debug('Tool function-calling response received')
        content = await get_content_from_response(response)
        log.debug('Tool function-calling content length=%d', len(content) if content else 0)
    ...
    body_metadata = ensure_metadata_dict(body.get('metadata'))
    body['metadata'] = body_metadata

    if skip_files and 'files' in body_metadata:
...
def get_audio_format_from_data_url(data_url: str) -> str:
    content_type = data_url.split(';', 1)[0].replace('data:', '').lower()
    audio_format = content_type.split('/', 1)[1] if '/' in content_type else 'wav'
...
async def normalize_input_audio_part(part: dict) -> Optional[dict]:
    input_audio = part.get('input_audio')
    if not isinstance(input_audio, dict):
        return None

    audio_data = input_audio.get('data')
    audio_format = (input_audio.get('format') or '').lower()
    if not audio_data:
        return None
    if audio_format in ('wav', 'mp3'):
        return part

    content_type = f'audio/{audio_format or "webm"}'
    if audio_format == 'mp4':
    ...
    converted_data_url = await get_openai_audio_base64_from_data_url(f'data:{content_type};base64,{audio_data}')
    if not converted_data_url or ',' not in converted_data_url:
        return None

    return {
        'type': 'input_audio',
        'input_audio': {
            'data': converted_data_url.split(',', 1)[1],
            'format': get_audio_format_from_data_url(converted_data_url),
        },
    }


async def normalize_input_audio_parts(content: list[dict]) -> list[dict]:
    normalized = []
    for part in content:
        ...
        normalized_part = await normalize_input_audio_part(part)
        if normalized_part:
...
async def build_multimodal_file_parts(files: list[dict]) -> list[dict]:
    parts = []
    ...
    for file in files:
        ...
        content_type = (
            ...
            or file.get('meta', {}).get('content_type')
            or file.get('file', {}).get('meta', {}).get('content_type')
            or ''
        )
        url = file.get('url') or ''
        ...
        if file_type == 'audio' or content_type.startswith('audio/'):
            audio_data_url = url if url.startswith('data:audio/') else None
            if audio_data_url:
                audio_data_url = await get_openai_audio_base64_from_data_url(audio_data_url)
            if not audio_data_url and file.get('id'):
                audio_data_url = await get_audio_base64_from_file_id(file.get('id'))

            if not audio_data_url or ',' not in audio_data_url:
                continue

            audio_format = get_audio_format_from_data_url(audio_data_url)
...
def is_context_file_item(file: dict) -> bool:
    content_type = (
        file.get('content_type')
        or file.get('meta', {}).get('content_type')
        or file.get('file', {}).get('meta', {}).get('content_type')
    ...
    return file_type in ('doc', 'text', 'note', 'chat', 'folder', 'collection') or (
        file_type == 'file' and not content_type.startswith('image/') and not content_type.startswith('audio/')
    )


async def inject_multimodal_files_into_messages(messages: list[dict]) -> None:
    for message in messages:
        file_parts = await build_multimodal_file_parts(message.get('files', []))
        if message.get('role') == 'user' and file_parts:
            ...
            if isinstance(text_content, str):
                ...
                message['content'] = [*text_content, *file_parts]
        if isinstance(message.get('content'), list):
            message['content'] = await normalize_input_audio_parts(message['content'])
        # Files have either been incorporated into content or intentionally
        # excluded from the LLM payload to avoid RAG treating audio as a source.
        message.pop('files', None)


def _message_has_audio_payload(message: dict | None) -> bool:
    ...
    for file in message.get('files') or []:
        ...
        if file.get('type') == 'audio' or content_type.startswith('audio/'):
            return True

    return False


def _message_already_in_payload(messages: list[dict], user_message: dict) -> bool:
    message_id = user_message.get('id')
    if message_id and any(isinstance(message, dict) and message.get('id') == message_id for message in messages):
        return True

    if _message_has_audio_payload(user_message):
        return any(
            isinstance(message, dict) and message.get('role') == 'user' and _message_has_audio_payload(message)
            for message in messages
        )
    ...
    if user_content is not None:
        return any(
            isinstance(message, dict) and message.get('role') == 'user' and message.get('content') == user_content
            for message in messages
        )
...
def ensure_current_user_message_in_messages(form_data: dict, metadata: dict) -> None:
    ...
    if _message_already_in_payload(messages, user_message):
        return

    messages.append(
        {k: copy.deepcopy(v) for k, v in user_message.items() if k in ('id', 'role', 'content', 'output', 'files')}
    )


VOICE_MESSAGE_DEFAULT_PROMPT = (
    '附件音频是用户对你说的话。请把语音内容当作用户直接输入的消息来理解并回复，'
    '并使用与语音相同的语言回答。不要转写、复述或描述语音内容，除非用户明确要求。'
)


def apply_voice_message_default_prompt(messages: list[dict]) -> None:
    """Treat a bare voice recording as the user's spoken message.

    Without instruction, audio-capable models tend to transcribe or describe
    attached audio. When the latest user message carries audio and no typed
    text, tell the model to answer the speech as if it were typed. If the user
    typed anything alongside (e.g. an explicit "识别语音内容"), their text wins
    ...
    if not isinstance(content, list):
        return
    if not any(isinstance(p, dict) and p.get('type') == 'input_audio' for p in content):
        return
    text_parts = [p for p in content if isinstance(p, dict) and p.get('type') == 'text']
    if any((p.get('text') or '').strip() for p in text_parts):
        return
    if text_parts:
        text_parts[0]['text'] = VOICE_MESSAGE_DEFAULT_PROMPT
    else:
        content.insert(0, {'type': 'text', 'text': VOICE_MESSAGE_DEFAULT_PROMPT})
...
async def chat_completion_files_handler(
    ...
    sources = []
    metadata = ensure_metadata_dict(body.get('metadata'))
    ...
    if files := metadata.get('files', None):
        ...
        queries = []
        is_need_generate_query = False
        if is_need_generate_query:
            queries = []
...
def apply_params_to_form_data(form_data, model):
    params = form_data.pop('params', {}) or {}
...
def build_root_messages_from_metadata(form_data: dict, metadata: dict) -> list[dict]:
    messages = []
...
async def process_chat_payload(request, form_data, user, metadata, model):
    ...
    log.debug(
        'Prepared chat request model=%s messages=%d',
        form_data.get('model'),
    ...
    is_root_message = metadata.get('has_parent_id') and metadata.get('parent_id') is None

    if is_root_message:
        form_data['messages'] = build_root_messages_from_metadata(form_data, metadata)
    elif chat_id and user_message_id and not chat_id.startswith('local:') and not chat_id.startswith('channel:'):
        ...
        if db_messages:
            ...
            # Inject multimodal files into content parts (mirrors frontend logic)
    ...
    ensure_current_user_message_in_messages(form_data, metadata)
    await inject_multimodal_files_into_messages(form_data.get('messages', []))
    ...
    payload_features = form_data.pop('features', None)
    ...
    if not any(
        ...
        # Default mode: a voice recording is the user's spoken message — the
        # model should reply to it, not transcribe it.
        apply_voice_message_default_prompt(form_data.get('messages') or [])
    ...
    if files:
        files = [file for file in files if is_context_file_item(file)]
    ...
    if payload_tools is None:
        ...
        terminal_capability = model_capabilities.get('terminal', True)
    ...
    file_context_enabled = model_capabilities.get('file_context', True)
...
async def get_system_oauth_token(request, user):
    ...
    try:
        ...
        log.error('Error getting OAuth token (%s)', type(e).__name__)
    return oauth_token
    ...
    if event_emitter:
        ...
        async def response_handler(response, events):
            ...
            try:
                ...
                model_meta = model_info.get('meta') if isinstance(model_info.get('meta'), dict) else {}
                model_capabilities = (
                    model_meta.get('capabilities') if isinstance(model_meta.get('capabilities'), dict) else {}
                )
                citations_enabled = model_capabilities.get('citations', True)
