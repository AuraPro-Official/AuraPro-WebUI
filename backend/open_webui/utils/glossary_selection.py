...
def apply_conversation_glossary(
    settings: dict[str, Any], selection: Any
) -> dict[str, Any]:
    ...
    if mode == 'fixed':
        ...
        resolved['target_lang'] = glossary.get('target_lang') or glossary.get(
            'glossary_lang'
        )
        return resolved

    source_fallback = str(
        resolved.get('smart_source_lang') or resolved.get('source_lang') or ''
    )
    target_fallback = str(
        resolved.get('smart_target_lang')
        or resolved.get('target_lang')
        or resolved.get('glossary_lang')
        or ''
    )
    resolved['glossary_mode'] = 'smart'
    resolved['smart_source_lang'] = _clean_language(
        selection.get('source_lang'), source_fallback
    )
    resolved['smart_target_lang'] = _clean_language(
        selection.get('target_lang'), target_fallback
    )
    return resolved
