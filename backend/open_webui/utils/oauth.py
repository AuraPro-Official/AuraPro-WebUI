...
from open_webui.retrieval.web.security import validate_url
...
def _oauth_response_fields(value) -> list[str]:
...
try:
    ...
    log.error('Error initializing Fernet with provided key (%s)', type(e).__name__)
    raise
        ...
        async with aiohttp.ClientSession(trust_env=True) as session:
            async with session.post(
                ...
                try:
                    ...
                    try:
                        log.error(
                            'Dynamic client registration failed at %s with status %s',
                            registration_url,
                    ...
                    log.error('Error parsing dynamic client registration response: %s', type(e).__name__)
...
class OAuthClientManager:
    ...
    async def get_oauth_token(self, user_id: str, client_id: str, force_refresh: bool = False):
        ...
        try:
            ...
            log.error(
                'Error getting OAuth token for user %s (%s)',
                user_id,
                type(e).__name__,
            )
            return None
    ...
    async def _refresh_token(self, session) -> dict:
        ...
        try:
            ...
            log.error(
                'Error refreshing token for session %s (%s)',
                session.id,
                type(e).__name__,
            )
            return None
            ...
            async with aiohttp.ClientSession(trust_env=True) as session_http:
                async with session_http.post(
                    ...
                    if r.status == 200:
                        ...
                        await r.read()
                        log.error('Token refresh failed for client_id %s with status %s', client_id, r.status)
                        return None

        except Exception as e:
            log.error(
                'Exception during token refresh for client_id %s (%s)',
                client_id,
                type(e).__name__,
            )
            return None
    ...
    async def handle_callback(self, request, client_id: str, user_id: str, response):
        ...
        try:
            ...
            if token and not token.get('access_token'):
                ...
                log.error(
                    'Invalid token response for client_id %s; fields=%s',
                    client_id,
                    _oauth_response_fields(token),
                )
                token = None
            ...
            if token:
                try:
                    ...
                    log.error(
                        'Failed to store OAuth session server-side (%s)',
                        type(e).__name__,
                    )
            else:
...
class OAuthManager:
            ...
            async with aiohttp.ClientSession(trust_env=True) as session_http:
                async with session_http.post(
                    ...
                    if r.status == 200:
                        ...
                        await r.read()
                        log.error('Token refresh failed for provider %s with status %s', provider, r.status)
                        return None

        except Exception as e:
            log.error(
                'Exception during token refresh for provider %s (%s)',
                provider,
                type(e).__name__,
            )
            return None
    ...
    async def update_user_groups(self, user, user_data, default_permissions, db=None):
        ...
        log.debug(f'All groups available in AuraPro: {[g.name for g in all_available_groups]}')
    ...
    async def handle_callback(self, request, provider, response, db=None):
        ...
        try:
            ...
            try:
                ...
                if type(token_exc).__name__ != 'BadSignatureError':
                    raise
            ...
            if not user_data:
                log.warning(
                    'OAuth callback failed for provider %s; user data is missing; token fields=%s',
                    provider,
                    _oauth_response_fields(token),
                )
                raise HTTPException(400, detail=ERROR_MESSAGES.INVALID_CRED)
            ...
            if not sub:
                log.warning(
                    'OAuth callback failed for provider %s; subject is missing; user fields=%s',
                    provider,
                    _oauth_response_fields(user_data),
                )
                raise HTTPException(400, detail=ERROR_MESSAGES.INVALID_CRED)
            ...
            if not email:
                ...
                if provider == 'github':
                    ...
                    log.warning(
                        'OAuth callback failed for provider %s; email is missing; user fields=%s',
                        provider,
                        _oauth_response_fields(user_data),
                    )
                    raise HTTPException(400, detail=ERROR_MESSAGES.INVALID_CRED)
            ...
            if (
                ...
                log.warning('OAuth callback rejected an email outside the allowed domains for provider %s', provider)
                raise HTTPException(400, detail=ERROR_MESSAGES.INVALID_CRED)
                ...
                if auth_config.OAUTH_UPDATE_EMAIL_ON_LOGIN:
                    ...
                    if email_claim:
                        ...
                        if new_email and new_email.lower() != user.email.lower():
                            ...
                            if existing_user:
                                log.error(
                                    'Cannot update email for user %s because it is already taken.',
                                    user.id,
                                )
                            else:
    ...
    async def handle_backchannel_logout(self, request, db=None):
        ...
        try:
            ...
            log.warning(
                'Back-channel logout: cannot decode logout_token (%s)',
                type(e).__name__,
            )
            return JSONResponse(
                status_code=400,
        ...
        for provider_name in OAUTH_PROVIDERS:
            ...
            try:
                ...
                log.debug(
                    'Back-channel logout: error checking provider %s (%s)',
                    provider_name,
                    type(e).__name__,
                )
                continue
        ...
        try:
            ...
            log.warning(
                'Back-channel logout: invalid logout_token (%s)',
                type(e).__name__,
            )
            return JSONResponse(
                status_code=400,
            ...
            log.error(
                'Back-channel logout: error validating logout_token (%s)',
                type(e).__name__,
            )
            return JSONResponse(
                status_code=400,
