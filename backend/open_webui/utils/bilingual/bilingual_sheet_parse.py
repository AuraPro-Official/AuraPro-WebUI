...
def parse_lang_pair(header_cell: str) -> tuple[str, str]:
    ...
    if not m:
        raise GoogleBilingualImportError(
            f"表头单元格 '{header_cell}' 不是合法的语言对格式（应形如 zh-ja / zh-en）。"
        )
...
def parse_doc_table_pairs(
    html: str, skip_first_table_row: bool = False
) -> list[tuple[str, str]]:
...
def parse_sheet_html(html: str) -> tuple[list[tuple[str, str, int]], list[dict[str, str]]]:
    ...
    if not table:
        raise GoogleBilingualImportError(
            "未能读取到表格内容，请确认该 Google Sheet 已设置为"
            "“知道链接的任何人可查看”。"
        )
...
async def import_bilingual_from_google_sheet(
    ...
    async with aiohttp.ClientSession() as session:
        ...
        for row_idx, row in enumerate(rows):
            ...
            primary_paragraphs = [
                p.langs.get(resolved_primary, "") for p in file_result.paragraphs
            ]
