...
async def load_tool_module_by_id(tool_id, content=None):
    ...
    # Dependencies are checked lazily when a tool is first loaded instead of
    # blocking every application startup.
    ...
    try:
        ...
        # This module is administrator-authored code and is the plugin execution boundary.
...
async def load_function_module_by_id(function_id: str, content: str | None = None):
    ...
    frontmatter = extract_frontmatter(content)
    # Dependencies are checked lazily when a function is first loaded.
    await asyncio.to_thread(install_frontmatter_requirements, frontmatter.get('requirements', ''))
    ...
    try:
        ...
        # User tool code reaches this boundary only when explicitly enabled by an administrator.
        exec(content, module.__dict__)  # nosec B102
        frontmatter = extract_frontmatter(content)
        log.info(f'Loaded module: {module.__name__}')
...
_requirements_lock = threading.Lock()


def _split_requirements(requirements: str) -> list[str]:
    result: list[str] = []
    current: list[str] = []
    bracket_depth = 0
    for character in requirements.replace('\r', '\n'):
        if character == '[':
            bracket_depth += 1
        elif character == ']':
...
def _validate_requirements(requirements: str) -> list[str]:
    raw_requirements = _split_requirements(requirements)
    if len(raw_requirements) > 64:
        raise ValueError('A tool or function may declare at most 64 packages.')

    validated = []
    for raw_requirement in raw_requirements:
        if len(raw_requirement) > 256:
            raise ValueError('A package requirement is too long.')
        try:
            requirement = Requirement(raw_requirement)
        except InvalidRequirement as exc:
            raise ValueError(f'Invalid package requirement: {raw_requirement}') from exc
        if requirement.url:
            raise ValueError('Direct URL, VCS, and local package requirements are not allowed.')
        validated.append(str(requirement))
    return validated
...
def install_frontmatter_requirements(requirements: str):
    ...
    if requirements:
        req_list = _validate_requirements(requirements)
        with _requirements_lock:
            new_reqs = [req for req in req_list if req not in _installed_requirements]
            if not new_reqs:
                return

            log.info('Installing %d declared tool/function package(s).', len(new_reqs))
            try:
                subprocess.run(
                ...
                log.exception('Error installing declared tool/function packages.')
                raise
