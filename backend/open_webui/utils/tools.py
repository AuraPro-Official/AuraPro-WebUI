...
MAX_TOOL_SERVER_SPEC_BYTES = 8 * 1024 * 1024
...
async def get_tools(request: Request, tool_ids: list[str], user: UserModel, extra_params: dict) -> dict[str, dict]:
    ...
    if not isinstance(extra_params.get('__metadata__'), dict):
...
async def get_builtin_tools(
    ...
    def get_model_capability(name: str, default: bool = True) -> bool:
        capabilities = model_meta.get('capabilities') if isinstance(model_meta.get('capabilities'), dict) else {}
    ...
    def is_builtin_tool_enabled(category: str) -> bool:
        builtin_tools = model_meta.get('builtinTools') if isinstance(model_meta.get('builtinTools'), dict) else {}
    ...
    model_knowledge = model_meta.get('knowledge') if isinstance(model_meta.get('knowledge'), list) else []
    # Merge folder-attached knowledge so builtin tools can search it
    folder_knowledge = extra_params['__metadata__'].get('folder_knowledge')
    if folder_knowledge:
...
async def get_tool_server_data(url: str, headers: dict | None) -> dict[str, Any]:
    ...
    try:
        ...
        async with aiohttp.ClientSession(timeout=timeout, trust_env=True) as session:
            async with session.get(url, headers=_headers, ssl=AIOHTTP_CLIENT_SESSION_TOOL_SERVER_SSL) as response:
                if response.status != 200:
                    ...
                    raise RuntimeError(f'Tool server returned HTTP {response.status}')

                content_length = response.headers.get('content-length')
                if content_length and int(content_length) > MAX_TOOL_SERVER_SPEC_BYTES:
                    raise ValueError('Tool server specification is too large')
                raw_content = await response.content.read(MAX_TOOL_SERVER_SPEC_BYTES + 1)
                if len(raw_content) > MAX_TOOL_SERVER_SPEC_BYTES:
                    raise ValueError('Tool server specification is too large')
        ...
        log.exception('Could not fetch tool server specification (%s)', type(err).__name__)
        raise RuntimeError('Unable to fetch tool server specification') from err

    path_count = len(res.get('paths', {})) if isinstance(res, dict) else 0
    log.debug('Fetched tool server specification paths=%d', path_count)
    return res
...
async def get_tool_servers_data(servers: list[dict[str, Any]]) -> list[dict[str, Any]]:
        ...
        if server.get('config', {}).get('enable') and server.get('type', 'openapi') == 'openapi':
            ...
            if spec_type == 'url':
                ...
                try:
                    ...
                    log.error('Error parsing JSON spec for tool server %s (%s)', id, type(e).__name__)
            ...
            if task:
                tasks.append(task)
                server_entries.append((id, idx, server, info, token))
    ...
    results = []
    for (id, idx, server, info, _), response in zip(server_entries, responses):
        if isinstance(response, Exception):
            log.error('Failed to connect to OpenAPI tool server id=%s', id)
            continue

        # Guard against invalid or non-OpenAPI specs (e.g., MCP-style configs)
        if not isinstance(response, dict) or 'paths' not in response:
            log.warning("Invalid OpenAPI spec for tool server id=%s: missing 'paths'", id)
            continue
