...
def normalize_sherpa_audio_base_url(value: str | None) -> str:
...
async def apply_default_sherpa_audio_config() -> None:
    keys = (
        'audio.stt.engine',
        'audio.stt.model',
    ...
    audio_config = await Config.get_many(*keys)
    updates = {}

    stt_engine = str(audio_config.get('audio.stt.engine') or '').strip().lower()
    if stt_engine not in {'', 'web', 'openai', 'sherpa', 'multimodal'}:
        stt_engine = 'sherpa'
        updates['audio.stt.engine'] = stt_engine
    if stt_engine == 'sherpa':
        updates.update(
            {
                'audio.stt.model': audio_config.get('audio.stt.model') or 'sherpa-asr',
                'audio.stt.openai.api_base_url': normalize_sherpa_audio_base_url(
                    audio_config.get('audio.stt.openai.api_base_url')
                ),
                'audio.stt.openai.api_key': audio_config.get('audio.stt.openai.api_key') or AURAPRO_SHERPA_API_KEY,
            }
        )

    tts_engine = str(audio_config.get('audio.tts.engine') or '').strip().lower()
    if tts_engine not in {'', 'openai', 'sherpa'}:
        tts_engine = 'sherpa'
        updates['audio.tts.engine'] = tts_engine
    if tts_engine == 'sherpa':
        updates.update(
            {
                'audio.tts.model': audio_config.get('audio.tts.model') or 'sherpa-tts',
                'audio.tts.voice': audio_config.get('audio.tts.voice') or '0',
                'audio.tts.openai.api_base_url': normalize_sherpa_audio_base_url(
                    audio_config.get('audio.tts.openai.api_base_url')
                ),
                'audio.tts.openai.api_key': audio_config.get('audio.tts.openai.api_key') or AURAPRO_SHERPA_API_KEY,
            }
        )
...
class SPAStaticFiles(StaticFiles):
    async def get_response(self, path: str, scope):
        try:
            ...
            return self._with_cache_headers(response)
    ...
    @staticmethod
    def _with_cache_headers(response):
...
async def lifespan(app: FastAPI):
    ...
    await Config.upsert(
        {
            ...
            'ui.enable_message_rating': False,
        }
    )
    ...
    try:
        from open_webui.utils.glossary_translation import ensure_glossary_settings_persisted

        await ensure_glossary_settings_persisted()
    except Exception as e:
        log.warning(f'Failed to persist glossary settings at startup: {e}')
...
async def chat_completion(
    ...
    try:
        ...
        context_size = form_data.get('params', {}).get('num_ctx')
        ...
        if model_info_params.get('num_ctx') is not None and context_size is None:
            context_size = model_info_params.get('num_ctx')
        ...
        has_parent_id = 'parent_id' in form_data
        is_new_chat = has_parent_id and form_data['parent_id'] is None and not form_data.get('chat_id')
...
async def generate_messages(
    ...
    All models configured in AuraPro are accessible via this endpoint.
    ...
    try:
        ...
        async with aiohttp.ClientSession(timeout=timeout, trust_env=True) as session:
            async with session.get(
                'https://api.github.com/repos/AuraPro-Official/AuraPro-WebUI/releases/latest',
...
async def get_current_usage(user=Depends(get_verified_user)):
    """
    Get current usage statistics for AuraPro.
...
async def register_client(request, client_id: str) -> bool:
    ...
    try:
        ...
        log.error(
            'Failed to persist updated OAuth client info for tool server %s (%s)',
            client_id,
            type(e).__name__,
        )
        return False
...
async def serve_cache_file(
    ...
    try:
        file_path = (cache_root / path).resolve(strict=True)
    except (OSError, RuntimeError):
        raise HTTPException(status_code=404, detail='File not found')
    if not file_path.is_relative_to(cache_root) or not file_path.is_file():
        raise HTTPException(status_code=404, detail='File not found')
