...
from open_webui.env import (
    ...
    ENABLE_USER_TOOL_CODE_EXECUTION,
)
    ...
    try:
        ...
        async with get_ssrf_safe_session() as session:
            async with session.get(
                ...
                if resp.status != 200:
                    raise HTTPException(status_code=502, detail='Failed to fetch the tool')
                content = await resp.content.read(2 * 1024 * 1024 + 1)
                if len(content) > 2 * 1024 * 1024:
                    raise HTTPException(status_code=413, detail='The remote tool is too large')
                if not content:
                    raise HTTPException(status_code=400, detail='No data received from the URL')
                data = content.decode(resp.charset or 'utf-8')
        return {
            'name': tool_name,
            'content': data,
        }
        ...
        raise HTTPException(status_code=400, detail='The tool URL or response is invalid') from exc
    except Exception as exc:
        log.warning('Failed to fetch remote tool: %s', type(exc).__name__)
        raise HTTPException(
            status_code=502,
...
async def create_new_tools(
    ...
    if user.role != 'admin' and (
        not ENABLE_USER_TOOL_CODE_EXECUTION
        or not (
...
async def update_tools_by_id(
    ...
    try:
        ...
        log.debug('Updating tool id=%s specs=%d', id, len(specs))
