...
CHAT_CONFIG_KEYS = {
    ...
    'CONTEXT_COMPACTION_THRESHOLD_PERCENT': 'chat.context_compaction.threshold_percent',
...
class ChatConfigForm(BaseModel):
    ...
    CONTEXT_COMPACTION_THRESHOLD_PERCENT: int = 75
...
async def set_chat_config(form_data: ChatConfigForm, user=Depends(get_admin_user)):
    ...
    threshold_percent = min(95, max(10, int(form_data.CONTEXT_COMPACTION_THRESHOLD_PERCENT or 75)))
    await Config.upsert(
        chat_config_updates(
            {
                **form_data.model_dump(),
                'CONTEXT_COMPACTION_TOKEN_THRESHOLD': threshold,
                'CONTEXT_COMPACTION_THRESHOLD_PERCENT': threshold_percent,
            }
        )
    )
    return await get_chat_config_values()
