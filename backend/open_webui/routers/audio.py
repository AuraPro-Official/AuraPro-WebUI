...
from open_webui.config import (
    AURAPRO_SHERPA_API_KEY,
    AURAPRO_SHERPA_AUDIO_BASE_URL,
    CACHE_DIR,
...
def transcode_audio_to_mp3(audio_data: bytes, content_type_header: str, output_path: str) -> bool:
    ...
    # pydub needs stdlib audioop (removed in Python 3.13); the project
    # supports Python <3.13 and loads it only when conversion is required.
    from pydub import AudioSegment
...
def normalize_tts_engine(engine: str | None):
    value = str(engine or '').strip().lower()
    if value == 'web':
        return ''
    return value if value in {'', 'openai', 'sherpa'} else ''


def normalize_stt_engine(engine: str | None):
    value = str(engine or '').strip().lower()
    return value if value in {'', 'web', 'openai', 'sherpa', 'multimodal'} else ''


def normalize_sherpa_audio_base_url(value: str | None):
    url = str(value or '').strip().rstrip('/')
    if not url or 'api.openai.com' in url.lower():
        url = AURAPRO_SHERPA_AUDIO_BASE_URL
...
async def update_audio_config(request: Request, form_data: AudioConfigUpdateForm, user=Depends(get_admin_user)):
    tts_config = form_data.tts.model_dump()
    tts_config['ENGINE'] = normalize_tts_engine(tts_config.get('ENGINE'))
    if tts_config['ENGINE'] == 'sherpa':
        tts_config['OPENAI_API_BASE_URL'] = normalize_sherpa_audio_base_url(tts_config.get('OPENAI_API_BASE_URL'))
        tts_config['OPENAI_API_KEY'] = tts_config.get('OPENAI_API_KEY') or AURAPRO_SHERPA_API_KEY
        tts_config['MODEL'] = tts_config.get('MODEL') or 'sherpa-tts'
        tts_config['VOICE'] = tts_config.get('VOICE') or '0'

    stt_config = form_data.stt.model_dump()
    stt_config['ENGINE'] = normalize_stt_engine(stt_config.get('ENGINE'))
    if stt_config['ENGINE'] == 'sherpa':
        stt_config['OPENAI_API_BASE_URL'] = normalize_sherpa_audio_base_url(stt_config.get('OPENAI_API_BASE_URL'))
        stt_config['OPENAI_API_KEY'] = stt_config.get('OPENAI_API_KEY') or AURAPRO_SHERPA_API_KEY
        stt_config['MODEL'] = stt_config.get('MODEL') or 'sherpa-asr'

    await Config.upsert(
        {
            **config_updates(tts_config, TTS_CONFIG_KEYS),
            **config_updates(stt_config, STT_CONFIG_KEYS),
        }
    )

    if stt_config['ENGINE'] == '':
...
async def _transcribe_whisper(request, file_path, languages, file_dir, id):
    ...
    explicit_language = languages[0] if languages else None
...
async def _transcribe_sherpa(request, file_path, filename, metadata, file_dir, id, user=None):
    ...
    if explicit_language:
        ...
        payload['language_candidates'] = metadata['language_candidates']

    if metadata.get('stream_id'):
        payload['stream_id'] = metadata['stream_id']
    if metadata.get('chunk_index') is not None:
        payload['chunk_index'] = str(metadata['chunk_index'])
    ...
    try:
        ...
        if response is not None:
            try:
                ...
                if message:
                    detail = f'External: {message}'
            except Exception:
                detail = f'External: {exc}'
        raise HTTPException(status_code=getattr(response, 'status', 500), detail=detail) from exc
    finally:
        audio_file.close()

    async with aiofiles.open(os.path.join(file_dir, f'{id}.json'), 'w') as file:
...
async def transcription_handler(request, file_path, metadata, user=None):
    ...
    if engine == '':
        ...
        return await _transcribe_sherpa(request, file_path, filename, metadata, file_dir, id, user)
    elif engine == 'openai':
    ...
    raise HTTPException(status_code=400, detail=f'Unsupported STT engine: {engine}')
...
async def transcription(
    ...
    language_candidates: Optional[str] = Form(None),
    stream_id: Optional[str] = Form(None),
    chunk_index: Optional[int] = Form(None),
    reset_stream: Optional[bool] = Form(None),
    ...
    try:
        ...
        try:
            ...
            if language_candidates:
                metadata['language_candidates'] = language_candidates
            if stream_id:
                metadata['stream_id'] = stream_id
            if chunk_index is not None:
                metadata['chunk_index'] = chunk_index
            if reset_stream is not None:
                metadata['reset_stream'] = reset_stream
        ...
        if not base_url.startswith('https://api.openai.com'):
            ...
            try:
                ...
                try:
                    ...
                    available_models = (
                        [{'id': await Config.get('audio.tts.model') or 'sherpa-tts'}]
                        if engine == 'sherpa'
                        else [{'id': 'tts-1'}, {'id': 'tts-1-hd'}]
                    )
        else:
    ...
    if engine in {'openai', 'sherpa'}:
        ...
        if not base_url.startswith('https://api.openai.com'):
            try:
                ...
                if engine == 'sherpa':
                    voice = str(await Config.get('audio.tts.voice') or '0')
                    return {voice: voice}
