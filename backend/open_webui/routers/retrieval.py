...
from open_webui.env import (
    ...
    RAG_RERANKING_MODEL_DEVICE_TYPE,
...
def get_ef(
    ...
    def load_embedding_model():
        ...
        try:
            ...
            if RAG_EMBEDDING_MODEL_DEVICE_TYPE == 'cuda':
                import torch

                if not torch.cuda.is_available():
                    raise RuntimeError('RAG CUDA acceleration is enabled, but PyTorch cannot access CUDA')
                allocated = torch.cuda.memory_allocated() / 1024**3
                reserved = torch.cuda.memory_reserved() / 1024**3
                total = torch.cuda.get_device_properties(0).total_memory / 1024**3
                log.info(
                    f'GPU Memory: {allocated:.2f}GB allocated, {reserved:.2f}GB reserved, {total:.2f}GB total available'
                )
    ...
    return LazyModel(load_embedding_model, f'embedding model {embedding_model}', log)
...
def get_rf(
            ...
            if engine == 'external':
                ...
                try:
                    rf = sentence_transformers.CrossEncoder(
                        get_model_path(reranking_model, auto_update),
                        device=RAG_RERANKING_MODEL_DEVICE_TYPE,
    ...
    return LazyModel(load_reranking_model, f'reranking model {reranking_model}', log)
...
##########################################
#
# Bilingual Files Cache Management
# 缓存双语文件信息以提升查询性能
#
##########################################


class BilingualFilesCache:
    """双语文件缓存管理器，减少数据库查询"""

    def __init__(self, max_entries: int = 32):
        self._cache: dict = {}  # {collection_name: {'data': grouped_data, 'timestamp': time}}
    ...
    def _prune(self) -> None:
        now = datetime.now()
        expired = [
            key for key, entry in self._cache.items() if now > entry['timestamp'] + timedelta(seconds=self._cache_ttl)
        ]
        for key in expired:
    ...
    def is_valid(self, collection_name: str) -> bool:
        """检查缓存是否有效"""
        if collection_name not in self._cache:
            return False

        cache_entry = self._cache[collection_name]
    ...
    def get(self, collection_name: str) -> Optional[dict]:
        """获取缓存数据"""
        if self.is_valid(collection_name):
            return self._cache[collection_name]['data']
        return None

    def set(self, collection_name: str, data: dict) -> None:
        """设置缓存数据"""
        self._cache.pop(collection_name, None)
        self._prune()
        self._cache[collection_name] = {'data': data, 'timestamp': datetime.now()}
...
# 全局缓存实例
_bilingual_cache = BilingualFilesCache()
...
async def unload_embedding_model(request: Request):
    ...
    if config.RAG_EMBEDDING_ENGINE == '':
        ...
        embedding_model = request.app.state.ef
        was_loaded = not isinstance(embedding_model, LazyModel) or embedding_model.is_loaded
        if isinstance(embedding_model, LazyModel):
        ...
        if was_loaded:
            ...
            gc.collect()
        if was_loaded and RAG_EMBEDDING_MODEL_DEVICE_TYPE == 'cuda':
            import torch
...
async def update_rag_config(request: Request, form_data: ConfigForm, user=Depends(get_admin_user)):
    ...
    if config.RAG_RERANKING_ENGINE == '':
        ...
        reranking_model = request.app.state.rf
        was_loaded = not isinstance(reranking_model, LazyModel) or reranking_model.is_loaded
        if isinstance(reranking_model, LazyModel):
            reranking_model.unload()
        ...
        if was_loaded:
            ...
            gc.collect()
        if was_loaded and RAG_RERANKING_MODEL_DEVICE_TYPE == 'cuda':
            import torch
...
def get_transformers_tokenizer(request: Request, config: RetrievalConfig):
    if config.RAG_TOKENIZER_MODEL:
        ...
        tokenizer_model, tokenizer_revision = resolve_huggingface_revision(
            tokenizer_model,
...
def save_docs_to_vector_db(
    ...
    try:
        ...
        embedding_timeout = RAG_EMBEDDING_TIMEOUT or 600  # Default to 10 minutes if not set
        log.info(
            f'Starting embedding generation: timeout={embedding_timeout}s, texts={len(texts)}, engine={config.RAG_EMBEDDING_ENGINE}, model={config.RAG_EMBEDDING_MODEL}'
        )
        ...
        if embedding is None:
            try:
                ...
                raise HTTPException(
                    ...
                    detail=f'Embedding generation timeout after {embedding_timeout} seconds. Please check your embedding service (Ollama/OpenAI) is running and responsive, or increase RAG_EMBEDDING_TIMEOUT environment variable.',
                )
            except Exception as e:
                log.error(f'Error during embedding generation: {type(e).__name__}: {e}')
                raise
...
async def process_text(
    ...
    text_content = form_data.content
    log.debug('Processing text content length=%d', len(text_content))
...
class GoogleSheetImportRequest(BaseModel):
    sheet_url: str = Field(..., description='Google Sheet 的分享链接（需公开可查看）')
    primary_lang: Optional[str] = Field(
        default=None,
        description='用于向量化 embedding 的主语言代码，如 zh。不传则取第一个语言对列的源语言',
    )
    skip_first_table_row: bool = Field(
        default=False,
        description="若 Google 文档内表格第一行是'原文|译文'这样的说明性表头，设为 true 可跳过该行",
    )
...
@router.post('/process/bilingual/google-sheet/preview')
async def preview_google_sheet(
    payload: GoogleSheetPreviewRequest,
    user=Depends(get_verified_user),
):
    import aiohttp
    from open_webui.utils.bilingual.bilingual_sheet_parse import (
        GoogleBilingualImportError,
        fetch_sheet_html,
        parse_sheet_html,
    )
    ...
    try:
        async with aiohttp.ClientSession() as session:
            sheet_html = await fetch_sheet_html(session, payload.sheet_url)
        lang_pair_columns, rows = parse_sheet_html(sheet_html)
    except GoogleBilingualImportError as e:
        raise HTTPException(status_code=400, detail=str(e))
    except Exception as e:
        raise HTTPException(status_code=500, detail=f'解析表格时出现未知错误: {e}')
    ...
    return {
        'languagePairs': [f'{src}-{tgt}' for src, tgt, _ in lang_pair_columns],
        'languages': languages,
        'rowCount': len(rows),
        'rows': rows,
    }
...
@router.post('/process/bilingual/google-sheet')
async def import_google_sheet(
    payload: GoogleSheetImportRequest,
    user=Depends(get_verified_user),
):
    """
    完整导入：解析表格 + 并发抓取每个 Google 文档正文，返回可直接喂给知识库入库流程的数据。
    """
    from open_webui.utils.bilingual.bilingual_sheet_parse import (
        GoogleBilingualImportError,
        import_bilingual_from_google_sheet,
    )

    try:
        result = await import_bilingual_from_google_sheet(
            sheet_url=payload.sheet_url,
            primary_lang=payload.primary_lang,
            skip_first_table_row=payload.skip_first_table_row,
        )
    except GoogleBilingualImportError as e:
        raise HTTPException(status_code=400, detail=str(e))
    except Exception as e:  # noqa: BLE001
        raise HTTPException(status_code=500, detail=f'导入过程中出现未知错误: {e}')

    if result.totalFiles == 0:
        raise HTTPException(status_code=400, detail='未能从表格中解析出任何有效数据行。')
...
async def process_bilingual_epub(
    ...
    max_file_size_mb = await Config.get('rag.file.max_size') or 512
    max_file_bytes = int(max_file_size_mb) * 1024 * 1024
    max_expanded_bytes = min(max_file_bytes * 5, 2 * 1024 * 1024 * 1024)
    temp_epub_dict = {}
    try:
        extraction_dict = {}
        for file in files:
            ...
            if lang in temp_epub_dict:
                raise ValueError(f'Duplicate EPUB language: {lang}')

            with tempfile.NamedTemporaryFile(delete=False, suffix='.epub') as tmp_file:
                temp_epub_dict[lang] = tmp_file.name
                uploaded_bytes = 0
                while chunk := await file.read(1024 * 1024):
                    uploaded_bytes += len(chunk)
                    if uploaded_bytes > max_file_bytes:
                        raise ValueError(f'EPUB file exceeds the {max_file_size_mb} MB limit')
                    tmp_file.write(chunk)

            if uploaded_bytes == 0:
                raise ValueError(f'EPUB file is empty: {file.filename}')

            try:
                with zipfile.ZipFile(temp_epub_dict[lang], 'r') as archive:
                    if len(archive.infolist()) > 10000:
                        raise ValueError('EPUB contains too many archive entries')
                    total_expanded = sum(info.file_size for info in archive.infolist())
                    if total_expanded > max_expanded_bytes:
                        raise ValueError('Expanded EPUB content exceeds the safety limit')
                    if any(
                        info.file_size > 10 * 1024 * 1024 and info.file_size > max(info.compress_size, 1) * 200
                        for info in archive.infolist()
                    ):
                        raise ValueError('EPUB has an unsafe compression ratio')
        ...
        if primaryLang not in extraction_dict:
            raise ValueError(f'Primary language EPUB was not provided: {primaryLang}')
        reference_result = extraction_dict.pop(primaryLang)

        result = ProcessBilingualForm(
            collection_name=collection_name,
            files=[],
            languages=langs,
            primaryLang=primaryLang,
            totalFiles=reference_result.get('chapter_count', 0),
        )
        ...
        for index, chapter in enumerate(chapters):
            ...
            for lang, extraction_result in extraction_dict.items():
                if index >= len(extraction_result['chapters']):
                    raise ValueError(f'EPUB chapter count does not match the primary language: {lang}')
                ex_chapter = extraction_result['chapters'][index]
                files_langs[lang] = ex_chapter['content']

            file = BilingualFile(
                baseName=chapter['title'],
                id=f'{chapter["title"]}__p0',
                langs=files_langs,
                primaryLang=primaryLang,
                primaryText=chapter['content'],
            )
        ...
        log.error(f'Error processing EPUB: {str(e)}')
        raise HTTPException(status_code=500, detail=f'Error processing EPUB: {str(e)}')
    finally:
        for lang, epub_path in temp_epub_dict.items():
            try:
...
class ProcessBilingualForm(BaseModel):
    collection_name: Optional[str] = None
    files: list[BilingualFile]
    languages: list[str]
    primaryLang: str
...
async def process_bilingual(
    ...
    collection_name = form_data.collection_name
    if collection_name is None:
        combined = '\n'.join(f.primaryText for f in form_data.files)
        collection_name = calculate_sha256_string(combined)
    else:
        await _validate_collection_access([collection_name], user, access_type='write')
    ...
    def _query_existing_ids():
        ...
        result = collection.get(
            where={'bilingual_id': {'$in': all_bilingual_ids}},
            include=['metadatas'],
        )
    ...
    for file in files_to_process:
        ...
        items.append(
            {
                'src_text': primary_text,
                'tgt_texts': other_langs,
                'src_lang': primary_lang,
                'metadata': base_meta,
            }
        )
...
class ListBilingualFilesResponse(BaseModel):
    collection_name: str
    files: list[BilingualFileSummary]
    total: int = 0
    skip: int = 0
    limit: int = 50


@router.get('/process/bilingual/files', response_model=ListBilingualFilesResponse)
async def list_bilingual_files(
    collection_name: str,
    skip: int = Query(0, ge=0),
    limit: int = Query(50, ge=1, le=1000),
    user=Depends(get_verified_user),
):
    await _validate_collection_access([collection_name], user, access_type='read')

    # 先检查缓存
    cached_data = _bilingual_cache.get(collection_name)
    if cached_data:
        files = cached_data['files']
        total = cached_data['total']
        log.debug(f'Using cached bilingual files for {collection_name}')
    else:
        # 缓存未命中，需要查询数据库
        def _query_and_group_bilingual_files():
            """
            优化的查询函数：
            - 使用较小的 batch size (50) 以提高响应速度
            - 在流式处理中进行分组，避免大内存占用
            - 只查询指定数量的记录
            """
            try:
            ...
            batch_size = 50  # 使用更小的 batch size 提高响应速度
            offset = 0
            max_records = 10000  # 最多查询 10000 条记录
                        ...
                        for meta in metadatas:
                            ...
                            if bilingual_id not in grouped:
                                ...
                                grouped[bilingual_id] = {
                                    ...
                                    'primary_lang': meta.get('primary_lang', ''),
                                    'languages': languages,
                                    'sentence_count': 0,
                                }
                            grouped[bilingual_id]['sentence_count'] += 1

                        offset += batch_size

                    except Exception as e:
                        log.error(f'Error querying batch at offset {offset}: {e}')
                        break

            except Exception as e:
                log.error(f'Error in _query_and_group_bilingual_files: {e}')
                return {}, 0

            return grouped, len(grouped)

        # 在线程池中执行查询
        grouped, total = await run_in_threadpool(_query_and_group_bilingual_files)

        # 构建文件列表并排序
        files = [BilingualFileSummary(**v) for v in grouped.values()]
        files.sort(key=lambda f: f.base_name)

        # 缓存结果
        cache_data = {'files': files, 'total': len(files)}
        _bilingual_cache.set(collection_name, cache_data)
        log.debug(f'Cached bilingual files for {collection_name}: {total} groups')

    # 进行分页
    paginated_files = files[skip : skip + limit]

    return ListBilingualFilesResponse(
        collection_name=collection_name,
...
@router.delete('/process/bilingual/file')
async def delete_bilingual_file(
    collection_name: str,
    bilingual_id: str,
    user=Depends(get_verified_user),
):
    """删除某个 bilingual_id 对应的所有句子级记录"""
    await _validate_collection_access([collection_name], user, access_type='write')
    ...
    def _delete():
        try:
            ...
            return False
        collection.delete(where={'bilingual_id': {'$eq': bilingual_id}})
        return True

    deleted = await run_in_threadpool(_delete)
    if not deleted:
        raise HTTPException(status_code=404, detail='Collection or bilingual_id not found')

    # 删除成功后清除缓存
    _bilingual_cache.invalidate(collection_name)

    return {'status': True, 'bilingual_id': bilingual_id}
...
class SentenceAlignItem(BaseModel):
    ...
    langs: dict[str, str]
    langs_modified: dict[str, bool]
    align_score: float


class ParagraphAlignItem(BaseModel):
    para_index: int
    para_text: str
    sentences: list[SentenceAlignItem]


class GetBilingualAlignResponse(BaseModel):
    bilingual_id: str
    primary_lang: str
    languages: list[str]
...
def _locate_offset(para_text: str, sentence_text: str, cursor: int) -> tuple[int, int]:
    ...
    if idx == -1:
        start = cursor
        end = cursor + len(find_text)
        return start, end
    return idx, idx + len(find_text)


@router.get('/process/bilingual/{bilingual_id}', response_model=GetBilingualAlignResponse)
async def get_bilingual_align(
    bilingual_id: str,
    collection_name: str,
    user=Depends(get_verified_user),
):
    ...
    collection = VECTOR_DB_CLIENT.client.get_collection(collection_name)
    result = collection.get(where={'bilingual_id': {'$eq': bilingual_id}}, include=['metadatas', 'documents'])

    if result is None or not result.get('ids', []):
        raise HTTPException(status_code=404, detail='No aligned sentences found for this bilingual_id')
...
class UpdateGlossaryWordsForm(BaseModel):
    collection_name: str
    bilingual_id: str
    lang: str
...
@router.get('/bilingual/{bilingual_id}/words')
async def get_bilingual_words(
    bilingual_id: str,
    collection_name: str,
    user=Depends(get_verified_user),
):
    ...
    def _query_metadata():
        try:
            ...
            return None
        result = collection.get(
            where={'bilingual_id': bilingual_id},
            include=['metadatas'],
            limit=1,
        )
        metadatas = result.get('metadatas', []) or []
    ...
    metadata = await run_in_threadpool(_query_metadata)
    if not metadata:
    ...
    for lang, pairs in words_raw.items():
        ...
        for idx, pair in enumerate(pairs or []):
            source = pair[0] if len(pair) > 0 else ''
...
@router.put('/bilingual/words')
async def update_bilingual_words(
    form_data: UpdateGlossaryWordsForm,
    user=Depends(get_verified_user),
):
    """
    保存某个双语文件、某个目标语言的术语表（整体覆盖式保存）。
    前端把编辑后的完整术语列表提交上来，服务端用它覆盖该 bilingual_id
    下所有 chunk 的 metadata["words"][lang]，保证同一文件所有 chunk 数据一致。
    """
    await _validate_collection_access([form_data.collection_name], user, access_type='write')

    def _update():
        try:
            collection = VECTOR_DB_CLIENT.client.get_collection(form_data.collection_name)
        except Exception:
            return None

        result = collection.get(
            where={'bilingual_id': form_data.bilingual_id},
            include=['metadatas'],
        )
        ...
        new_pairs = [[t.source, t.target] for t in form_data.terms]
    ...
    updated_count = await run_in_threadpool(_update)
    if updated_count is None:
        raise HTTPException(status_code=404, detail='未找到该双语文件对应的数据')

    # 术语表变了不影响检索缓存本身（向量内容没变），但为保险起见仍然让缓存失效，
    # 避免任何依赖 metadata 快照的读路径拿到旧数据。
    _bilingual_cache.invalidate(form_data.collection_name)

    return {
        'status': True,
        'bilingual_id': form_data.bilingual_id,
        'lang': form_data.lang,
        'updated_chunks': updated_count,
        'term_count': updated_count,
    }
...
async def process_web(
    ...
    try:
        ...
        log.debug('Loaded web content length=%d', len(content))
...
def _removed_web_search_provider(*args, **kwargs):
    raise RuntimeError('Web search support is not included in AuraPro.')


async def _removed_async_web_search_provider(*args, **kwargs):
    raise RuntimeError('Web search support is not included in AuraPro.')


search_ollama_cloud = search_perplexity_search = search_yacy = _removed_web_search_provider
search_brave_llm_context = search_kagi = search_mojeek = _removed_web_search_provider
search_bocha = search_serply = search_duckduckgo = search_tavily = _removed_web_search_provider
search_exa = search_searchapi = search_serpapi = search_jina = _removed_web_search_provider
search_bing = search_azure = search_perplexity = search_microsoft_web_iq = _removed_web_search_provider
search_sougou = search_firecrawl = search_external = search_yandex = _removed_web_search_provider
search_youcom = search_linkup = _removed_web_search_provider
search_searxng = search_google_pse = search_brave = _removed_async_web_search_provider
search_serpstack = search_serper = search_serphouse = _removed_async_web_search_provider


async def search_web(request: Request, engine: str, query: str, user=None) -> list:
...
async def process_web_search(request: Request, form_data: SearchForm, user=Depends(get_verified_user)):
    ...
    try:
        ...
        log.debug('Web search produced %d unique URL(s)', len(urls))
