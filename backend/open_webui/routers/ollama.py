...
async def create_model(
    ...
    log.debug('Creating Ollama model name=%s', form_data.model)
    url = (await Config.get('ollama.base_urls', []))[url_idx]
...
async def embed(
    ...
    log.debug('Generating Ollama batch embeddings model=%s', form_data.model)
...
async def embeddings(
    ...
    log.debug('Generating Ollama embeddings model=%s', form_data.model)
    await check_model_access(user, await Models.get_model_by_id(form_data.model), BYPASS_MODEL_ACCESS_CONTROL)
    await validate_ollama_backend_idx(request, form_data.model, url_idx, user)
...
def parse_huggingface_url(hf_url: str) -> str | None:
    ...
    try:
        filename = os.path.basename(unquote(urlparse(hf_url).path).replace('\\', '/')).strip(' .')
...
async def download_file_stream(
    ...
    async with session.get(
        ...
        if response.status not in {200, 206}:
            raise RuntimeError(f'Model download failed with status {response.status}.')

        append = current_size > 0 and response.status == 206
        if not append:
            current_size = 0
        content_length = response.headers.get('content-length')
        total_size = current_size + int(content_length) if content_length else None
        ...
        with open(file_path, 'ab' if append else 'wb') as f:
            async for data in response.content.iter_chunked(chunk_size):
                ...
                progress = round((current_size / total_size) * 100, 2) if total_size else 0
                yield f'data: {json.dumps({"progress": progress, "completed": current_size, "total": total_size})}\n\n'

        hashed = await asyncio.to_thread(_calculate_file_sha256, file_path, chunk_size)
        ...
        with open(file_path, 'rb') as blob:
            async with session.post(
                ...
                ssl=AIOHTTP_CLIENT_SESSION_SSL,
                timeout=aiohttp.ClientTimeout(total=6 * 60 * 60, sock_read=300),
            ) as blob_resp:
                if not blob_resp.ok:
                    raise RuntimeError('Ollama: Could not create blob, Please try again.')

        os.remove(file_path)
        yield f'data: {json.dumps({"done": True, "blob": f"sha256:{hashed}", "name": file_name})}\n\n'


def _calculate_file_sha256(file_path: str, chunk_size: int) -> str:
...
async def download_model(
    ...
    download_key = hashlib.sha256(form_data.url.encode('utf-8')).hexdigest()[:16]
    file_path = os.path.join(UPLOAD_DIR, f'.ollama-download-{download_key}-{file_name}')
    return StreamingResponse(
...
async def upload_model(
    ...
    if not filename:
        raise HTTPException(status_code=400, detail='A valid model filename is required.')
    os.makedirs(UPLOAD_DIR, exist_ok=True)
    file_descriptor, file_path = tempfile.mkstemp(prefix='.ollama-upload-', dir=UPLOAD_DIR)
    ...
    async def file_process_stream():
        ...
        try:
            if total_size == 0:
                raise ValueError('The uploaded model file is empty.')

            # Stage 2: hash the file and emit SSE progress in one pass.
            digest = hashlib.sha256()
            ...
            # Stage 3: stream the blob to Ollama without loading it into memory.
            session = await get_session()
            ...
            log.debug('Creating uploaded Ollama model name=%s', model)
