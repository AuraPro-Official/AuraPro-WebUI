...
@router.get('/{id}/export-with-vectors')
...
async def import_knowledge_with_vectors(
    ...
    file = form.get('file')
    knowledge_id = form.get('knowledge_id')
    if not file or not knowledge_id:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail='File and knowledge ID are required.',
        )

    knowledge = await Knowledges.get_knowledge_by_id(knowledge_id, db=db)
    if not knowledge:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail=ERROR_MESSAGES.NOT_FOUND,
