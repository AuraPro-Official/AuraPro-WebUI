...
async def signin(
    ...
    if WEBUI_AUTH_TRUSTED_EMAIL_HEADER:
        ...
        admin_password = hmac.new(
            ...
            b'aurapro-system-account-v1',
            hashlib.sha256,
        ).hexdigest()

        existing_user = await Users.get_user_by_email(admin_email.lower(), db=db)
        if existing_user:
            user = await Auths.authenticate_user(
                admin_email.lower(),
            ...
            if user is None:
                # One-time migration for installations created with the legacy
                # fixed credential. This password is never accepted remotely
                # when authentication is enabled.
                legacy_password = 'admin'  # nosec B105
                user = await Auths.authenticate_user(
                    admin_email.lower(),
                    lambda pw: verify_password(legacy_password, pw),
                    db=db,
                )
                if user:
                    await Auths.update_user_password_by_id(
                        user.id,
...
async def get_admin_details(
    ...
    if await Config.get('auth.admin.show'):
        ...
        log.debug('Admin details requested')
...
async def token_exchange(
    ...
    """
    Exchange an external OAuth provider token for an AuraPro JWT.
