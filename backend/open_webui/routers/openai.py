...
def get_microsoft_entra_id_access_token():
    ...
    try:
        ...
        log.error(
            'Error getting Microsoft Entra ID access token (%s)',
            type(e).__name__,
        )
        return None
    ...
    try:
        ...
        try:
            ...
            raise HTTPException(
                ...
                detail=detail if detail else 'AuraPro: Server Connection Error',
            )
    ...
    if url_idx is None:
        ...
        async with aiohttp.ClientSession(
            ...
            try:
                ...
                raise HTTPException(status_code=500, detail='AuraPro: Server Connection Error')
            except Exception as e:
...
def _normalize_stored_item(item: dict) -> dict:
    ...
    AuraPro stores extra bookkeeping fields (``id``, ``status``,
...
async def proxy(path: str, request: Request, user=Depends(get_verified_user)):
    ...
    try:
        ...
        raise HTTPException(
            status_code=r.status if r else 500,
            detail='AuraPro: Server Connection Error',
        )
    finally:
        if not streaming:
            await cleanup_response(r)
