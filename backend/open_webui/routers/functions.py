    ...
    try:
        ...
        async with get_ssrf_safe_session() as session:
            async with session.get(
                ...
                if resp.status != 200:
                    raise HTTPException(status_code=502, detail='Failed to fetch the function')
                content = await resp.content.read(2 * 1024 * 1024 + 1)
                if len(content) > 2 * 1024 * 1024:
                    raise HTTPException(status_code=413, detail='The remote function is too large')
                if not content:
        ...
        raise HTTPException(status_code=400, detail='The function URL or response is invalid') from exc
    except Exception as exc:
        log.warning('Failed to fetch remote function: %s', type(exc).__name__)
        raise HTTPException(
            status_code=502,
            detail='Error fetching function',
        ) from exc
...
async def update_function_by_id(
    ...
    try:
        ...
        log.debug('Updating function id=%s type=%s', id, function_type)
