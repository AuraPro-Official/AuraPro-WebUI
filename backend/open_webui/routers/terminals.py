...
def _normalize_terminal_base_url(value: str) -> str | None:
...
async def proxy_terminal(
    ...
    base_url = _normalize_terminal_base_url(connection.get('url') or '')
    if not base_url:
        return JSONResponse({'error': 'Terminal server URL is invalid or not configured'}, status_code=503)
    ...
    if policy_id:
        target_url = f'{base_url}/p/{quote(str(policy_id), safe="")}/{safe_path}'
    ...
    try:
        upstream_response = await session.request(
            ...
            headers=headers,
            cookies=cookies,
            data=request.stream() if has_body else None,
        ...
        await session.close()
        log.exception('Terminal proxy request failed')
        return JSONResponse({'error': 'Terminal server request failed'}, status_code=502)
...
async def _resolve_authenticated_connection(ws: WebSocket, server_id: str):
    ...
    try:
        ...
        if len(raw) > 16_384:
            await ws.close(code=4001, reason='Authentication payload is too large')
            return None
...
async def ws_terminal(
    ...
    base_url = _normalize_terminal_base_url(connection.get('url') or '')
    if not base_url:
        await ws.close(code=4003, reason='Terminal server URL is invalid or not configured')
        return
    ...
    if policy_id:
        upstream_url = f'{ws_base}/p/{quote(str(policy_id), safe="")}/api/terminals/{safe_session_id}'
    else:
    ...
    try:
        ...
        log.exception('Terminal WebSocket proxy failed')
    finally:
        await session.close()
        if opened:
            await publish_event(
