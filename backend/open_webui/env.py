...
_default_model_device = 'cuda' if USE_CUDA.lower() == 'true' else ('mps' if sys.platform == 'darwin' else 'cpu')


def _model_device_from_env(name: str) -> str:
    device = os.getenv(name, _default_model_device).strip().lower()
    if device not in {'cpu', 'cuda', 'mps'}:
        print(f'Invalid {name}={device!r}; falling back to {_default_model_device}')
        return _default_model_device
    return device


# Device availability is validated when a local model is first used. Importing
# torch here adds several seconds to every backend start, even when RAG is idle.
RAG_EMBEDDING_MODEL_DEVICE_TYPE = _model_device_from_env('RAG_EMBEDDING_MODEL_DEVICE_TYPE')
RAG_RERANKING_MODEL_DEVICE_TYPE = _model_device_from_env('RAG_RERANKING_MODEL_DEVICE_TYPE')
DEVICE_TYPE = RAG_EMBEDDING_MODEL_DEVICE_TYPE
print(
    'PyTorch device selection deferred: '
    f'embedding={RAG_EMBEDDING_MODEL_DEVICE_TYPE}, reranking={RAG_RERANKING_MODEL_DEVICE_TYPE}'
)
...
if FROM_INIT_PY:
    try:
        PACKAGE_DATA = {'version': importlib.metadata.version('aurapro-webui')}
    except importlib.metadata.PackageNotFoundError:
        try:
            PACKAGE_DATA = {'version': importlib.metadata.version('open-webui')}
        except importlib.metadata.PackageNotFoundError:
            try:
                PACKAGE_DATA = json.loads((BASE_DIR / 'package.json').read_text(encoding='utf-8'))
            except Exception:
...
@lru_cache(maxsize=1)
def get_changelog() -> dict[str, dict]:
    """Parse the bundled changelog on first request, then reuse the result."""
    import pkgutil
    ...
    try:
        changelog_content = (BASE_DIR / 'CHANGELOG.md').read_text(encoding='utf8')
    except OSError:
        changelog_content = (pkgutil.get_data('open_webui', 'CHANGELOG.md') or b'').decode()

    soup = BeautifulSoup(markdown.markdown(changelog_content), 'html.parser')
    changelog: dict[str, dict] = {}

    for version in soup.find_all('h2'):
        heading = version.get_text().strip()
        version_label, separator, date = heading.partition(' - ')
        if not separator:
            log.warning('Ignoring malformed changelog heading: %s', heading)
            continue

        version_number = version_label.strip().strip('[]')
        if not version_number:
            continue
        version_data = {'date': date.strip()}
        current = version.find_next_sibling()

        while current and current.name != 'h2':
            if current.name == 'h3':
                section_title = current.get_text().lower()
                version_data[section_title] = parse_section(current.find_next_sibling('ul'))
            current = current.find_next_sibling()

        changelog[version_number] = version_data
...
def _sqlite_pragma_choice(name: str, default: str, allowed: set[str]) -> str:
    value = os.getenv(name, default).strip().upper()
    if value == '':
        return ''
    if value not in allowed:
        log.warning('%s has an invalid value; using %s.', name, default)
        return default
    return value


def _sqlite_pragma_integer(name: str, default: int, minimum: int, maximum: int) -> str:
    ...
    if parsed < minimum or parsed > maximum:
        log.warning('%s is outside the supported range; using %s.', name, default)
        parsed = default
    return str(parsed)


DATABASE_SQLITE_PRAGMA_SYNCHRONOUS = _sqlite_pragma_choice(
...
DATABASE_SQLITE_PRAGMA_BUSY_TIMEOUT = _sqlite_pragma_integer('DATABASE_SQLITE_PRAGMA_BUSY_TIMEOUT', 5000, 0, 600000)

# PRAGMA cache_size: negative value = KiB.  -65536 ≈ 64 MB page cache.
DATABASE_SQLITE_PRAGMA_CACHE_SIZE = _sqlite_pragma_integer(
    'DATABASE_SQLITE_PRAGMA_CACHE_SIZE', -65536, -2097152, 2097152
)

# PRAGMA temp_store: MEMORY (2) keeps temp tables and indices in RAM.
# Valid values: DEFAULT (0), FILE (1), MEMORY (2).
DATABASE_SQLITE_PRAGMA_TEMP_STORE = _sqlite_pragma_choice(
    'DATABASE_SQLITE_PRAGMA_TEMP_STORE',
    'MEMORY',
    {'DEFAULT', 'FILE', 'MEMORY', '0', '1', '2'},
)

# PRAGMA mmap_size (bytes): memory-mapped I/O size.  268435456 ≈ 256 MB.
# Set to 0 to disable mmap.
DATABASE_SQLITE_PRAGMA_MMAP_SIZE = _sqlite_pragma_integer('DATABASE_SQLITE_PRAGMA_MMAP_SIZE', 268435456, 0, 17179869184)
...
DATABASE_SQLITE_PRAGMA_JOURNAL_SIZE_LIMIT = _sqlite_pragma_integer(
    'DATABASE_SQLITE_PRAGMA_JOURNAL_SIZE_LIMIT', 67108864, -1, 17179869184
)
...
if WEBUI_SECRET_KEY == '':
    raise SystemExit(
        'WEBUI_SECRET_KEY is not set. It is required for session and encrypted configuration security.\n'
...
# Allows external apps to exchange OAuth tokens for Open WebUI tokens
...
ENABLE_PIP_INSTALL_FRONTMATTER_REQUIREMENTS = (
    os.getenv('ENABLE_PIP_INSTALL_FRONTMATTER_REQUIREMENTS', 'False').lower() == 'true'
)
ENABLE_USER_TOOL_CODE_EXECUTION = os.getenv('ENABLE_USER_TOOL_CODE_EXECUTION', 'False').lower() == 'true'
