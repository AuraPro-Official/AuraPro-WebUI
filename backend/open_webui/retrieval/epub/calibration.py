...
class LocalConceptCalibrationRunner:
    ...
    @staticmethod
    def _evaluate_one(*, endpoint: PrivateModelEndpoint, model: str, transport: LlamaCppTransport, prompt_profile: str, passage: Mapping[str, Any]) -> CalibrationItemReport:
        ...
        try:
            ...
            validation = ConceptPayloadValidation(False, 0, 0, _safe_reason(error))
        return CalibrationItemReport(passage_id, ordinal, toc_path, validation.valid, validation.concept_count, validation.mention_count, validation.reason)
