...
class SQLiteBatchRepository:
    ...
    def _ingest_section_graph(self, connection: Any, *, version_id: str, item: Any, payload: Mapping[str, Any]) -> None:
    ...
    def ingest_success(self, batch_job_id: str, custom_id: str, payload: Mapping[str, Any]) -> bool:
        ...
        with self._store._write() as connection:
            ...
            if job['job_kind'] == 'SECTION_GRAPH':
                self._ingest_section_graph(connection, version_id=job['version_id'], item=item, payload=payload)
            else:
                concepts = payload.get('concepts')
