...
class MariaDBVectorClient(VectorDBBase):
    ...
    def _build_filter_sql_qmark(self, expr: Any) -> Tuple[str, List[Any]]:
        ...
        for key, value in expr.items():
            if key.startswith('$'):
                continue
            json_expr = 'JSON_UNQUOTE(JSON_EXTRACT(vmetadata, ?))'
            json_path = f'$.{key}'
            if isinstance(value, dict) and '$in' in value:
                ...
                params.extend((json_path, str(value)))
        ...
        try:
            with self._connect() as conn:
                with conn.cursor() as cur:
                    ...
                    # The distance function is allowlisted and all filter values are bound parameters.
                    sql_template = """
                        SELECT
                          id,
                          text,
                          vmetadata,
                          {distance_function}(embedding, ?) AS dist
                        FROM document_chunk
                        WHERE {where_clause}
                        ORDER BY dist ASC
                        LIMIT ?
                    """
                    sql = sql_template.format(  # nosec B608
    ...
    def query(self, collection_name: str, filter: Dict[str, Any], limit: Optional[int] = None) -> Optional[GetResult]:
        ...
        with self._connect() as conn:
            with conn.cursor() as cur:
                ...
                # WHERE contains only fragments built by _build_filter_sql_qmark.
                sql = f'SELECT id, text, vmetadata FROM document_chunk WHERE {where}'  # nosec B608
        ...
        with self._connect() as conn:
            with conn.cursor() as cur:
                try:
                    ...
                    # WHERE contains only fixed clauses and parameter placeholders.
