...
DEFAULT_WEB_FETCH_MAX_BYTES = 100 * 1024 * 1024


def _web_fetch_limit(loader_config: dict) -> int:
    configured = loader_config.get('web_fetch_max_content_length')
    try:
        configured = int(configured) if configured is not None else 0
    except (TypeError, ValueError):
        configured = 0
    return configured if configured > 0 else DEFAULT_WEB_FETCH_MAX_BYTES


def _join_documents_with_limit(docs: list[Document], max_content_bytes: int) -> str:
    total_bytes = 0
    parts = []
    for doc in docs:
        ...
        if total_bytes > max_content_bytes:
            raise ValueError('Fetched web content exceeds the configured size limit')
        parts.append(part)
    return ' '.join(parts)
...
def _extract_text_from_binary_response(
    ...
    candidate_suffix = os.path.splitext(filename)[1].lower()
    suffix = candidate_suffix if re.fullmatch(r'\.[a-z0-9]{1,10}', candidate_suffix) else ''
    ...
    try:
        with tempfile.NamedTemporaryFile(suffix=suffix, delete=False) as tmp:
            ...
            if content_length and int(content_length) > max_content_bytes:
                raise ValueError('Fetched web file exceeds the configured size limit')
            downloaded_bytes = 0
            for chunk in response.iter_content(chunk_size=1024 * 1024):
                if not chunk:
                    continue
                downloaded_bytes += len(chunk)
                if downloaded_bytes > max_content_bytes:
                    raise ValueError('Fetched web file exceeds the configured size limit')
                tmp.write(chunk)
        ...
        content = _join_documents_with_limit(docs, max_content_bytes)
        return content, docs
    finally:
        if tmp_path:
            try:
...
def _get_content_from_url_sync(request, url: str, loader_config):
    from open_webui.retrieval.web.security import _SSRFSafeAdapter, validate_url

    # Validate URL before making any request (blocks private IPs, non-HTTP, filter list)
    validate_url(url)
    max_content_bytes = _web_fetch_limit(loader_config)
...
def get_enriched_texts(collection_result: GetResult) -> list[str]:
    ...
    for idx, text in enumerate(collection_result.documents[0]):
        ...
        # Add snippet metadata when available
...
def generate_openai_batch_embeddings(
    ...
    r = requests.post(
        ...
        headers=headers,
        json=json_data,
        timeout=(10, AIOHTTP_CLIENT_TIMEOUT),
    )
    r.raise_for_status()
    data = r.json()
    if 'data' in data:
...
def resolve_huggingface_revision(
    ...
    *,
    ...
    revision = None
    if '@' in model:
        model_name, requested_revision = model.rsplit('@', 1)
        if model_name and requested_revision:
            model = model_name
            revision = requested_revision

    if revision is None:
        revision = 'main' if local_files_only else HfApi().model_info(repo_id=model).sha
    if not revision:
        raise RuntimeError(f'Could not resolve an immutable revision for Hugging Face model {model}')
...
def get_model_path(model: str, update_model: bool = False):
    ...
    model, revision = resolve_huggingface_revision(
        model,
    ...
    try:
        ...
        log.error(
            'Cannot determine model snapshot path for %s (%s)',
            model,
            type(e).__name__,
        )
        if OFFLINE_MODE:
