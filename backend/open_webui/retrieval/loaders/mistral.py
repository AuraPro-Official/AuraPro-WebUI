...
class MistralLoader:
    ...
    def __init__(
        ...
        self.headers = {
            ...
            'User-Agent': 'AuraPro-MistralLoader/2.0',  # Helps API provider track usage
        }
    ...
    def _handle_response(self, response: requests.Response) -> Dict[str, Any]:
        ...
        try:
            ...
            log.error('Mistral request failed with HTTP status %s', response.status_code)
            raise
        except requests.exceptions.RequestException as req_err:
            log.error('Mistral request failed (%s)', type(req_err).__name__)
            raise
        except ValueError as json_err:  # Includes JSONDecodeError
            log.error('Mistral response JSON decode failed (%s)', type(json_err).__name__)
    ...
    async def _get_session(self):
        ...
        async with aiohttp.ClientSession(
            connector=connector,
            timeout=timeout,
            headers={'User-Agent': 'AuraPro-MistralLoader/2.0'},
