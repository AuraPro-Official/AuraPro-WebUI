...
class DatalabMarkerLoader:
    ...
    def check_marker_request_status(self, request_id: str) -> dict:
        ...
        try:
            ...
            log.debug('Marker API status check completed for request %s', request_id)
            return result
    ...
    def load(self) -> List[Document]:
        ...
        log.info('Sending file to Datalab Marker: %s', filename)
        ...
        if check_url:
            ...
            for _ in range(300):  # Up to 10 minutes
                ...
                try:
                    ...
                    log.error(
                        'Polling failed status=%s (%s)',
                        getattr(poll_response, 'status_code', 'unknown'),
                        type(e).__name__,
                    )
