...
try:
    ...
    PlaywrightURLLoader = BaseLoader
...
try:
    ...
    ExternalWebLoader = BaseLoader
    MicrosoftWebIQLoader = BaseLoader
    TavilyLoader = BaseLoader

    def scrape_firecrawl_url(*args, **kwargs):
        raise RuntimeError('Optional web loader support is not included in AuraPro')
...
def safe_validate_urls(urls: Sequence[str]) -> Sequence[str]:
    ...
    for url in urls:
        try:
            ...
            log.debug('Rejected unsafe URL (%s)', type(e).__name__)
    return valid_urls
...
def verify_ssl_cert(url: str) -> bool:
    ...
    try:
        ...
        for family, kind, protocol, _, address in infos:
            raw_sock = socket.socket(family, kind, protocol)
        ...
        log.warning('SSL verification failed (%s)', type(e).__name__)
        return False
        ...
        for url in self.web_paths:
            try:
                ...
                if self.continue_on_failure:
                    log.warning('Firecrawl extraction failed (%s)', type(e).__name__)
                    continue
                raise
...
class SafeTavilyLoader(BaseLoader, RateLimitMixin, URLProcessingMixin):
    ...
    def lazy_load(self) -> Iterator[Document]:
        ...
        for url in self.web_paths:
            try:
                ...
                log.warning('URL safety validation failed (%s)', type(e).__name__)
        ...
        try:
            ...
            if self.continue_on_failure:
                log.exception('Tavily extraction failed (%s)', type(e).__name__)
            else:
                raise e
    ...
    async def alazy_load(self) -> AsyncIterator[Document]:
        ...
        try:
            ...
            if self.continue_on_failure:
                log.exception('Tavily URL loading failed (%s)', type(e).__name__)
            else:
                raise e
...
class SafeMicrosoftWebIQLoader(BaseLoader, RateLimitMixin, URLProcessingMixin):
    ...
    async def alazy_load(self) -> AsyncIterator[Document]:
        try:
            ...
            if self.continue_on_failure:
                log.warning('Microsoft Web IQ loading failed (%s)', type(e).__name__)
            else:
                raise e
        ...
        with sync_playwright() as p:
            ...
            for url in self.urls:
                try:
                    ...
                    if self.continue_on_failure:
                        log.exception('Playwright URL loading failed (%s)', type(e).__name__)
                        continue
                    raise e
            browser.close()
...
class SafeWebBaseLoader(WebBaseLoader):
    def __init__(self, trust_env: bool = False, verify_ssl: bool = True, *args, **kwargs):
        ...
        async with aiohttp.ClientSession(trust_env=self.trust_env, connector=connector) as session:
            for i in range(retries):
                try:
                    ...
                    kwargs['ssl'] = False if not self.session.verify else AIOHTTP_CLIENT_SESSION_SSL
                    ...
                    if i == retries - 1:
                        raise
                    log.warning(
                        'Web fetch attempt %d/%d failed (%s); retrying',
                        i + 1,
                        retries,
                        type(e).__name__,
                    )
    ...
    def lazy_load(self) -> Iterator[Document]:
        for path in self.web_paths:
            try:
                if self.verify_ssl and not verify_ssl_cert(path):
                    raise ValueError(f'SSL certificate verification failed for {path}')

                soup = self._scrape(path, bs_kwargs=self.bs_kwargs)
                text = soup.get_text(**self.bs_get_text_kwargs)
                yield Document(page_content=text, metadata=extract_metadata(soup, path))
            except Exception as e:
                log.exception('Safe web page loading failed (%s)', type(e).__name__)
...
def get_web_loader(
    ...
    if not safe_urls:
        count = 1 if isinstance(urls, str) else len(urls)
        log.warning('All %d provided URL(s) were blocked or invalid', count)
        raise ValueError(ERROR_MESSAGES.INVALID_URL)
