...
def configure_epub_rag_inference_policy(app_state: Any, rag_config: Mapping[str, Any]) -> None:
    ...
    app_state.EPUB_RAG_EMBEDDING_PROFILE = embedding_profile if isinstance(embedding_profile, str) and embedding_profile else None
    app_state.EPUB_RAG_RERANKER_PROFILE = reranker_profile if isinstance(reranker_profile, str) and reranker_profile else None
...
def _embedding_policy(
    *, engine: str, endpoint: Any, trusted_hostnames: frozenset[str]
) -> ModelAvailability:
    if engine == "":
    ...
    *,
    ...
    if vector_backend is not None:
        try:
            ...
            vector_payload = _availability_payload(
                ModelAvailability.degraded("sqlite-vec", _safe_reason(error))
            )
    else:
