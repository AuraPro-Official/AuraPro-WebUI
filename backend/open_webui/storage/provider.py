...
class FileTooLargeError(ValueError):
    def __init__(self, max_bytes: int):
        super().__init__(f'File exceeds the configured limit of {max_bytes} bytes')
...
class LocalStorageProvider(StorageProvider):
    ...
    def upload_file(
        ...
        file_path = _safe_local_path(filename)
        temporary_path = file_path.with_name(f'.{file_path.name}.{uuid.uuid4().hex}.uploading')
        ...
        try:
            ...
            return UploadResult(path=str(file_path), size=size, sha256=digest.hexdigest())
        except Exception:
...
class S3StorageProvider(StorageProvider):
    ...
    def upload_file(
        ...
        local_result = LocalStorageProvider.upload_file(file, filename, tags, max_bytes)
        s3_key = '/'.join(part.strip('/') for part in (self.key_prefix, filename) if part)
        try:
            self.s3_client.upload_file(local_result.path, self.bucket_name, s3_key)
            if S3_ENABLE_TAGGING and tags:
                sanitized_tags = {
                    self.sanitize_tag_value(k)[:128]: self.sanitize_tag_value(v)[:256] for k, v in tags.items()
                }
...
class GCSStorageProvider(StorageProvider):
    ...
    def upload_file(
        ...
        try:
            ...
            return UploadResult(
                path='gs://' + self.bucket_name + '/' + filename,
                size=local_result.size,
                sha256=local_result.sha256,
            )
...
class AzureStorageProvider(StorageProvider):
    ...
    def upload_file(
        ...
        try:
            ...
            with open(local_result.path, 'rb') as contents:
                blob_client.upload_blob(contents, overwrite=True)
            return UploadResult(
                path=f'{self.endpoint}/{self.container_name}/{filename}',
                size=local_result.size,
                sha256=local_result.sha256,
            )
        except Exception as e:
