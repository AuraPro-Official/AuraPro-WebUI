...
# Handles secret key generation, optional Ollama/CUDA setup,
...
: "${USE_OLLAMA_DOCKER:=}" "${USE_CUDA_DOCKER:=}"
...
if [[ -n "${SPACE_ID:-}" ]]; then
  ...
  if [[ -n "${ADMIN_USER_EMAIL:-}" && -n "${ADMIN_USER_PASSWORD:-}" ]]; then
    ...
    WEBUI_SECRET_KEY="${WEBUI_SECRET_KEY:-}" \
      uvicorn open_webui.main:app --host "$HOST" --port "$PORT" --forwarded-allow-ips "${FORWARDED_ALLOW_IPS:-127.0.0.1,::1}" &
    webui_pid=$!
