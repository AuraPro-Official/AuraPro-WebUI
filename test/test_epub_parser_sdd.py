...
class TestEpubParserSDD(unittest.TestCase):
    def test_nav_precedence_fragment_breadcrumbs_and_faithful_units(self) -> None:
        ...
        self.assertEqual([passage.content_kind for passage in result.passages], ["paragraph", "heading", "paragraph", "list_item", "blockquote", "pre"])
    ...
    def test_nested_blocks_are_not_duplicated_and_fallback_is_conservative(self) -> None:
        ...
        self.assertEqual([(item.content_kind, item.content) for item in fallback.units], [("fallback", "plain content & punctuation。")])

        mixed = extract_xhtml_text("<body>before<p>paragraph</p>after</body>", "mixed.xhtml")
        self.assertEqual([(item.content_kind, item.content) for item in mixed.units], [("fallback", "before"), ("paragraph", "paragraph"), ("fallback", "after")])
