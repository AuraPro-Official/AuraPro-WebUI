<script>
	...
	const createActiveTabChannel = () => {
		if (typeof BroadcastChannel === 'undefined') {
			return null;
		}

		try {
			return new BroadcastChannel('active-tab-channel');
		} catch (error) {
			console.warn('BroadcastChannel is unavailable:', error);
			return null;
		}
	};

	const bc = createActiveTabChannel();
	...
	const desktopEventHandler = async (event) => {
		...
		if (event.type === 'action:activate' && event.data?.action) {
			desktopEvent.set(event);
			await goto('/');
			return;
		}
		...
		try {
			...
			} else if (event.type === 'audio:sherpa') {
				const audioConfig = await getAudioConfig(token);
				...
				if (event.data.action === 'add') {
					await updateAudioConfig(token, {
						tts: {
							...
							MODEL: 'sherpa-tts',
							VOICE: '0',
							OPENAI_API_BASE_URL: event.data.url,
							OPENAI_API_KEY: 'aurapro-local'
						},
						stt: {
							...audioConfig.stt,
							ENGINE: 'sherpa',
	...
	const windowMessageEventHandler = (event) => {
		...
		desktopEventHandler(data).catch((error) => {
			console.error('Window message handling failed:', error);
		});
	};
	...
	onMount(async () => {
		...
		if (window?.electronAPI) {
			try {
				...
				console.error('Desktop app info loading failed:', error);
			}
			...
			if (window.electronAPI.onEvent) {
				window.electronAPI.onEvent((event) => {
					desktopEventHandler(event).catch((error) => {
						console.error('Desktop event handling failed:', error);
					});
				});
			}
		}
