<script lang="ts">
	...
	import { AURAPRO_SETUP_WIZARD_VERSION, WEBUI_VERSION, WEBUI_API_BASE_URL } from '$lib/constants';
	import { compareVersion } from '$lib/utils';
	import { applyExtensionMode } from '$lib/utils/extension-modes';
	...
	let settingsModalPromise: Promise<
		typeof import('$lib/components/chat/SettingsModal.svelte')
	> | null = null;
	let setupWizardPromise: Promise<typeof import('$lib/components/SetupWizard.svelte')> | null =
		null;
	let changelogModalPromise: Promise<
		typeof import('$lib/components/ChangelogModal.svelte')
	> | null = null;
	let accountPendingPromise: Promise<
		typeof import('$lib/components/layout/Overlay/AccountPending.svelte')
	> | null = null;
	let updateInfoToastPromise: Promise<
		typeof import('$lib/components/layout/UpdateInfoToast.svelte')
	> | null = null;

	const loadSettingsModal = () =>
		(settingsModalPromise ??= import('$lib/components/chat/SettingsModal.svelte'));
	const loadSetupWizard = () =>
		(setupWizardPromise ??= import('$lib/components/SetupWizard.svelte'));
	const loadChangelogModal = () =>
		(changelogModalPromise ??= import('$lib/components/ChangelogModal.svelte'));
	const loadAccountPending = () =>
		(accountPendingPromise ??= import('$lib/components/layout/Overlay/AccountPending.svelte'));
	const loadUpdateInfoToast = () =>
		(updateInfoToastPromise ??= import('$lib/components/layout/UpdateInfoToast.svelte'));
	...
	onMount(async () => {
		...
		applyExtensionMode($settings?.defaultExtensionMode ?? '');
		if (($settings?.setupWizardVersion ?? 0) < AURAPRO_SETUP_WIZARD_VERSION) {
...
{#if $showSettings}
	{#await loadSettingsModal() then { default: SettingsModal }}
		<SettingsModal bind:show={$showSettings} />
	{/await}
{/if}

{#if $showSetupWizard}
	{#await loadSetupWizard() then { default: SetupWizard }}
		<SetupWizard bind:show={$showSetupWizard} />
	{/await}
{/if}

{#if $showChangelog}
	{#await loadChangelogModal() then { default: ChangelogModal }}
...
{#if version && compareVersion(version.latest, version.current) && ($settings?.showUpdateToast ?? true)}
	<div class=" absolute bottom-8 right-8 z-50" in:fade={{ duration: 100 }}>
		{#await loadUpdateInfoToast() then { default: UpdateInfoToast }}
			<UpdateInfoToast
				{version}
				on:close={() => {
...
{#if $user}
	<div class="app relative">
		...
		>
			{#if !['user', 'admin'].includes($user?.role)}
				{#await loadAccountPending() then { default: AccountPending }}
					<AccountPending />
				{/await}
			{:else}
				{#if localDBChats.length > 0}
