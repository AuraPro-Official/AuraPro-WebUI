<script lang="ts">
	...
	let RAG_EMBEDDING_ENGINE = 'sentence-transformers';
	...
	let lastDownloadedRerankingModel = '';
	...
	const setModelDownloadComplete = (model: string, type: 'embedding' | 'reranking') => {
		if (type === 'embedding') {
			lastDownloadedEmbeddingModel = model;
			embeddingDownloadDone = true;
			downloadProgress.embedding.current = 100;
		} else {
			lastDownloadedRerankingModel = model;
			rerankingDownloadDone = true;
			downloadProgress.reranking.current = 100;
		}
	};

	const resetModelDownloadState = (type: 'embedding' | 'reranking') => {
		if (type === 'embedding') {
			embeddingDownloadInProgress = false;
			downloadProgress.embedding.current = null;
			embeddingDownloadDone = false;
		} else {
			rerankingDownloadInProgress = false;
			downloadProgress.reranking.current = null;
			rerankingDownloadDone = false;
		}
	};

	const downloadModelWithProgress = async (model: string, type: 'embedding' | 'reranking') => {
		if (!model) {
			toast.error($i18n.t('Model URL is required for download.'));
			return;
		}

		if (type === 'embedding') {
			embeddingDownloadInProgress = true;
			downloadProgress.embedding.current = 0;
			embeddingDownloadDone = false;
		} else {
			rerankingDownloadInProgress = true;
			downloadProgress.reranking.current = 0;
			rerankingDownloadDone = false;
		}

		try {
			const response = await downloadHFModel(localStorage.token, { repo_id: model });
			if (!response?.body) {
				throw new Error($i18n.t('Unable to start model download.'));
			}
				...
				if (value) {
					...
					for (const line of lines) {
						...
						try {
							...
							console.log('[Download] Parsed data:', data);
							if (typeof data.completed === 'number' && typeof data.total === 'number') {
								const progress = Math.min(100, Math.round((data.completed / data.total) * 100));
								console.log(`[Download] Progress: ${progress}% (${data.completed}/${data.total})`);
								if (type === 'embedding') {
									downloadProgress.embedding.current = progress;
								} else {
									downloadProgress.reranking.current = progress;
								}
							}

							if (data.status && data.status.toLowerCase() === 'completed') {
								console.log('[Download] Download completed');
								setModelDownloadComplete(model, type);
							}
						} catch (error) {
							console.error('[Download] Parse error:', error, 'line:', line);
						}
					}
				}
			}
			...
			if (textBuffer.trim()) {
				try {
					...
					console.log('[Download] Final buffer data:', data);
				} catch (error) {
					console.error('[Download] Final parse error:', error);
				}
			}

			setModelDownloadComplete(model, type);
			toast.success($i18n.t('Model download completed successfully.'));
		} catch (error) {
			const message = error instanceof Error ? error.message : String(error);
			console.error('[Download] Error:', message);
			toast.error(message);
			resetModelDownloadState(type);
		} finally {
			if (type === 'embedding') {
				embeddingDownloadInProgress = false;
			} else {
								...
								{#if RAG_EMBEDDING_ENGINE === 'ollama'}
									<div class="flex w-full">
										...
										<button
											...
											on:click={downloadEmbeddingModel}
											disabled={embeddingDownloadInProgress}
										>
											{#if embeddingDownloadInProgress}
												<div class="self-center">
													<Spinner />
												</div>
											{:else}
										...
										{#if embeddingDownloadDone && lastDownloadedEmbeddingModel === RAG_EMBEDDING_MODEL}
											<CheckCircle class="w-4 h-4 text-green-500 self-center ml-2" />
										{/if}
									</div>

									{#if downloadProgress.embedding.current !== null}
										<div class="mt-2 text-xs text-gray-500 dark:text-gray-400">
											{$i18n.t('Download progress: {{progress}}%', {
												progress: downloadProgress.embedding.current
											})}
										</div>
									{/if}
								{:else}
									...
									<div class="">
										<div class="flex w-full">
											...
											<button
												type="button"
												class="px-2.5 bg-transparent text-gray-800 dark:bg-transparent dark:text-gray-100 rounded-lg transition"
												on:click={downloadRerankingModel}
												disabled={rerankingDownloadInProgress}
											>
												{#if rerankingDownloadInProgress}
													<div class="self-center">
														<Spinner />
													</div>
												{:else}
											...
											{#if rerankingDownloadDone && lastDownloadedRerankingModel === RAGConfig.RAG_RERANKING_MODEL}
												<CheckCircle class="w-4 h-4 text-green-500 self-center ml-2" />
											{/if}
										</div>

										{#if downloadProgress.reranking.current !== null}
											<div class="mt-2 text-xs text-gray-500 dark:text-gray-400">
												{$i18n.t('Download progress: {{progress}}%', {
													progress: downloadProgress.reranking.current
												})}
											</div>
										{/if}
									</div>
