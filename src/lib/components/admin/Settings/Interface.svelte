<script lang="ts">
	...
	const updateInterfaceHandler = async () => {
		chatConfig.CONTEXT_COMPACTION_THRESHOLD_PERCENT = Math.min(
			95,
			Math.max(10, Math.round(Number(chatConfig.CONTEXT_COMPACTION_THRESHOLD_PERCENT) || 75))
		);
		...
		<div class="  overflow-y-scroll scrollbar-hidden h-full pr-1.5">
			<div class="mb-3.5">
				...
				{#if chatConfig.ENABLE_CONTEXT_COMPACTION}
					<div class="mb-2.5">
						<div class=" mb-1 text-xs font-medium">{$i18n.t('Context Compaction Threshold')}</div>

						<Tooltip
							content={$i18n.t(
								'Older messages are summarized when estimated context exceeds this percentage of the actual context size.'
							)}
							placement="top-start"
						>
							<div class="flex items-center gap-2">
								<input
									...
									step="1"
									class="w-full rounded-lg py-2 px-4 text-sm bg-gray-50 dark:text-gray-300 dark:bg-gray-850 outline-hidden"
									bind:value={chatConfig.CONTEXT_COMPACTION_THRESHOLD_PERCENT}
								/>
