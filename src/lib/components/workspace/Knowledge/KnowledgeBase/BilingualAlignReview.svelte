    ...
    try {
        ...
        res.languages.forEach(lang => {
            if (lang !== res.primary_lang) {
                newEditableTerms[lang] = res.terms[lang]
                    ? res.terms[lang].map(t => ({ ...t }))
                    : [];
            }
        });
...
<div class="p-4 max-w-6xl mx-auto">
    {#if loading}
        <div class="text-sm text-gray-400 py-10 text-center">{$i18n?.t('Loading...') ?? '加载中...'}</div>
    {:else if errorMsg}
    ...
    {:else if data}
        ...
        <div class="flex gap-5 items-start">
                        ...
                        {#each renderParaSegments(para.para_text, para.sentences) as seg}
                            {#if seg.sentence}
                                <span
                                    ...
                                    on:click={(e) => openSentence(seg.sentence!, e.currentTarget)}
                                >{seg.text}</span>
                            {:else}
                                <span>{seg.text}</span>
                            {/if}
                ...
                {#if activeSentence}
                    ...
                    >
                        {#each data.languages.filter((l) => l !== data?.primary_lang) as lang}
                            ...
                            <div class="border-b last:border-b-0 border-gray-100 dark:border-gray-800 pb-2 last:pb-0">
                                <div class="flex items-center justify-between mb-1">
                                    <span class="text-[11px] font-medium text-gray-400 dark:text-gray-500 flex items-center gap-1">
                                        {lang.toUpperCase()}
                                        {#if modified}
                                            <span class="text-[10px] px-1 rounded bg-green-100 text-green-600 dark:bg-green-900/40 dark:text-green-300">
                                                {$i18n?.t('Edited') ?? '已修改'}
                                            </span>
                                        {/if}
                                    </span>
                                ...
                                {#if isEditing}
                                    ...
                                    <div class="flex justify-end gap-2 mt-1.5">
                                        ...
                                        <button
                                            ...
                                            on:click={saveEdit}
                                        >
                                            {savingLang === lang ? ($i18n?.t('Saving...') ?? '保存中...') : ($i18n?.t('Save') ?? '保存')}
                                        </button>
                                    </div>
                                {:else}
            ...
            {#if showGlossarySidebar}
                <div class="w-[360px] shrink-0 sticky top-4 max-h-[calc(100vh-6rem)] overflow-y-auto rounded-2xl border border-gray-100 dark:border-gray-800 bg-white dark:bg-gray-900 shadow-sm">
                    <div class="flex items-center justify-between px-4 py-3 border-b border-gray-100 dark:border-gray-800 sticky top-0 bg-white dark:bg-gray-900 z-10">
                    ...
                    <div class="p-3">
                        {#if glossaryLoading}
                            <div class="text-xs text-gray-400 py-8 text-center">{$i18n?.t('Loading...') ?? '加载中...'}</div>
                        ...
                        {:else if glossaryData}
                            <div class="text-[11px] text-gray-400 dark:text-gray-500 mb-3 leading-relaxed">
                                {$i18n?.t('Terms are automatically extracted from aligned sentence pairs. You can review and correct them here — edits are saved to this file only.')
                                    ?? '术语从对齐的句对中自动抽取，可在此核对修改；修改仅保存到当前文件。'}
                            </div>

                            {#each glossaryData.languages.filter((l) => l !== glossaryData?.primary_lang) as lang}
                                <div class="mb-4 rounded-xl border border-gray-100 dark:border-gray-800 overflow-hidden">
                                    <div class="flex items-center justify-between px-3 py-2 bg-gray-50 dark:bg-gray-850">
                                        ...
                                        <div class="flex items-center gap-1.5">
                                            {#if dirtyLangs[lang]}
                                                <span class="w-1.5 h-1.5 rounded-full bg-amber-400" title={$i18n?.t('Unsaved changes') ?? '有未保存的修改'} />
                                            {/if}
                                            <button
                                    ...
                                    {#if (editableTerms[lang] ?? []).length === 0}
                                        <div class="px-3 py-4 text-center text-[11px] text-gray-400">
                                            {$i18n?.t('No terms extracted for this language yet.') ?? '该语言暂无抽取出的术语。'}
                                        </div>
                                    {:else}
                                        <div class="divide-y divide-gray-50 dark:divide-gray-800">
                                        {#each editableTerms[lang] as term, idx (term.id)}
                                            <div class="flex items-center gap-1 px-2 py-1.5 hover:bg-gray-50/60 dark:hover:bg-gray-800/40 group">
                                                <!-- Source 输入框 -->
