<script lang="ts">
	...
	export let knowledgeType: string = 'general';
	let show = false;
</script>
...
>
	...
	<div slot="content">
		...
		{#if knowledgeType !== 'bilingual'}
			...
			{#if knowledgeType === 'bilingual'}
				<button
					class="select-none flex gap-2 items-center px-3 py-1.5 text-sm cursor-pointer hover:bg-blue-50 dark:hover:bg-blue-900/20 rounded-xl w-full text-blue-600 dark:text-blue-400"
					on:click={() => {
						show = false;
						onUpload({ type: 'bilingual' });
					}}
				>
					<svg
						xmlns="http://www.w3.org/2000/svg"
