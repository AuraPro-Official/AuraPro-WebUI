...
{#if show}
	...
	>
		...
		>
				...
				{#if step === 'select'}
					<div class="flex flex-col items-center justify-center gap-6 py-10">
						...
						<div class="grid grid-cols-1 md:grid-cols-3 gap-4 w-full max-w-3xl">
							<!-- Files Option -->
							<button
								class="p-6 rounded-xl border-2 transition-all hover:border-blue-500 hover:bg-blue-50 dark:hover:bg-blue-900/20 {importMode ===
								'files'
							...
							>
								<div class="flex flex-col items-center gap-3">
									<div
										class="w-16 h-16 rounded-2xl bg-blue-100 dark:bg-blue-900/40 flex items-center justify-center"
									>
										<svg
											xmlns="http://www.w3.org/2000/svg"
							...
							<!-- EPUB Option -->
							<button
								class="p-6 rounded-xl border-2 transition-all hover:border-purple-500 hover:bg-purple-50 dark:hover:bg-purple-900/20 {importMode ===
								'epub'
							...
							>
								<div class="flex flex-col items-center gap-3">
									<div
										class="w-16 h-16 rounded-2xl bg-purple-100 dark:bg-purple-900/40 flex items-center justify-center"
									>
										<svg
											xmlns="http://www.w3.org/2000/svg"
							...
							<!-- Google Sheet Option -->
							<button
								class="p-6 rounded-xl border-2 transition-all hover:border-emerald-500 hover:bg-emerald-50 dark:hover:bg-emerald-900/20 {importMode ===
								'google'
							...
							>
								<div class="flex flex-col items-center gap-3">
									<div
										class="w-16 h-16 rounded-2xl bg-emerald-100 dark:bg-emerald-900/40 flex items-center justify-center"
									>
										<svg
											xmlns="http://www.w3.org/2000/svg"
						...
						{#if importMode === 'files'}
							<div class="w-full max-w-md bg-gray-50 dark:bg-gray-800 rounded-xl p-4">
								<div class="grid grid-cols-2 gap-2">
									{#each [['article_zh.txt', 'article_en.txt'], ['chapter-zh.md', 'chapter-en.md']] as [a, b]}
										<div class="flex flex-col gap-1">
											<div
												class="text-xs font-mono bg-white dark:bg-gray-750 border border-gray-200 dark:border-gray-700 rounded px-2 py-1 text-gray-600 dark:text-gray-300"
											>
												{a}
											</div>
											<div
						...
						{:else if importMode === 'epub'}
							<div
								class="w-full max-w-md bg-purple-50 dark:bg-purple-900/20 rounded-xl p-4 border border-purple-200 dark:border-purple-700"
							>
								<div class="flex gap-2 mb-2">
									<svg
										class="w-4 h-4 text-purple-600 dark:text-purple-400 mt-0.5 shrink-0"
										fill="currentColor"
										viewBox="0 0 20 20"
									>
										<path
						...
						{:else if importMode === 'google'}
							<div class="w-full max-w-md flex flex-col gap-3">
								...
								>
									<div class="flex gap-2 mb-2">
										...
										<div class="text-xs text-emerald-700 dark:text-emerald-300">
											{$i18n?.t(
												'Sheet header cells must use "source-target" language pairs, e.g. zh-ja, zh-en. Each cell in a row is a link to a Google Doc containing a table (left column = original text, right column = translation), aligned row by row.'
											) ??
												'表格表头需为"源语言-目标语言"格式，如 zh-ja、zh-en。每个单元格是一个 Google 文档链接，文档内为表格（左列原文/右列译文），逐行对齐。'}
										</div>
									</div>
									<div class="text-xs text-emerald-600 dark:text-emerald-400 space-y-1">
										<p>
											✓ {$i18n?.t('Sheet and docs must be shared as "Anyone with the link"') ??
												'表格和文档都需公开分享（知道链接的任何人可查看）'}
										</p>
										<p>
											✓ {$i18n?.t('Automatically fetches and aligns each row') ??
												'自动抓取并按行对齐每一段'}
										</p>
									</div>
								</div>
								...
								<input
									...
									bind:value={sheetUrl}
									placeholder={$i18n?.t('Paste Google Sheet share link') ??
										'粘贴 Google 表格分享链接'}
									disabled={isProcessing}
								/>
				...
				{:else if step === 'preview'}
					...
					{#if alignWarnings.length > 0}
						{#each alignWarnings as w}
							<div
								class="mb-3 flex items-start gap-2 px-3 py-2.5 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-700 rounded-xl text-sm text-amber-700 dark:text-amber-300"
							>
					...
					<div class="border border-gray-200 dark:border-gray-700 rounded-xl overflow-hidden">
						...
						>
							<div
								class="bg-gray-50 dark:bg-gray-800 px-2 py-2 text-xs font-medium text-gray-400 text-center"
							>
								#
							</div>
							{#each detectedLanguages as lang}
								<div
									class="bg-gray-50 dark:bg-gray-800 px-3 py-2 text-xs font-semibold text-gray-600 dark:text-gray-300 flex items-center gap-1.5"
								>
									{#if lang === primaryLang}
						...
						<div class="max-h-64 overflow-y-auto divide-y divide-gray-100 dark:divide-gray-800">
							{#each paragraphRows.slice(0, 30) as row}
								...
								>
									<div
										class="bg-white dark:bg-gray-900 px-2 py-2 text-xs text-gray-400 text-center flex items-start justify-center pt-3"
									>
										{row.index + 1}
									</div>
									{#each detectedLanguages as lang}
										<div
											class="bg-white dark:bg-gray-900 px-3 py-2 text-xs text-gray-700 dark:text-gray-300 leading-relaxed"
										>
											{#if row.langs[lang]}
												{row.langs[lang].length > 120
			...
			<!-- Footer -->
			<div
				class="flex items-center justify-between px-6 py-4 border-t border-gray-100 dark:border-gray-800 shrink-0"
			>
				<button
				...
				<div class="flex gap-2">
					...
					{:else if step === 'confirm'}
						<button
							...
							on:click={() => {
								if (importMode === 'files' || importMode === 'google') handleConfirm();
								else handleEpubConfirm();
							}}
							disabled={isProcessing}
						>
							{#if isProcessing}
