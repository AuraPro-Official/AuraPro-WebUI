<script lang="ts">
	...
	let showBilingualContentModal = false;
	...
	const uploadBilingualFiles = async (data: BilingualImportData) => {
		...
		toast.info(
			$i18n.t('Starting bilingual import: {{count}} paragraph pairs', {
				count: files.length
			})
		);

		try {
			const result = await processBilingual(
				localStorage.token,
				knowledge.id,
				files,
				languages,
	...
	const handleUploadError = (error) => {
		if (error?.name === 'AbortError' || error?.message === 'Directory selection was cancelled') {
	...
	const collectDirectoryFiles = async (): Promise<DirectoryFileEntry[] | null> => {
		const isFileSystemAccessSupported = 'showDirectoryPicker' in window && !window.electronAPI;
...
<BilingualModal
	...
	onClose={() => {
		showBilingualContentModal = false;
	}}
/>
			...
			{:else}
				<div class="px-3.5 flex flex-1 items-center w-full space-x-2 py-0.5 pb-2">
					<div class="flex flex-1 items-center">
						...
						{#if knowledge?.write_access}
							<div>
								<AddContentMenu
									knowledgeType={knowledge?.meta?.knowledge_type ?? 'general'}
									onUpload={(data) => {
										...
										} else if (data.type === 'bilingual') {
											showBilingualContentModal = true;
										} else {
				...
				{#if knowledge?.meta?.knowledge_type === 'bilingual'}
					<BilingualFiles
						...
						{selectedFileId}
						onClick={(bilingualId) => {
							selectedFileId = bilingualId;
							goto(
								`/workspace/knowledge/bilingual-align/${bilingualId}?collection=${knowledge?.id ?? ''}`
							);
						}}
						onDelete={(bilingualId) => {
							selectedFileId = null;
							selectedFile = null;
						}}
					/>
