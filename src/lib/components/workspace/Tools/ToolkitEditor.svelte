...
class Tools:
    ...
    def calculator(
        ...
        try:
            ...
            result = self._calculate_node(ast.parse(equation, mode="eval").body)
    ...
    def _calculate_node(self, node):
        ...
        if isinstance(node, ast.UnaryOp) and type(node.op) in unary_operators:
            return unary_operators[type(node.op)](self._calculate_node(node.operand))
        if isinstance(node, ast.BinOp) and type(node.op) in binary_operators:
            left = self._calculate_node(node.left)
            right = self._calculate_node(node.right)
