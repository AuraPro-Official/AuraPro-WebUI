<script lang="ts">
	import type { SpreadsheetWorkbook } from '$lib/utils/spreadsheet';
	...
	let selectedTab = '';
	let excelWorkbook: SpreadsheetWorkbook | null = null;
	...
	const loadExcelContent = async () => {
		try {
			excelError = '';
			const [arrayBuffer, { parseSpreadsheet }] = await Promise.all([
				getFileContentById(item.id),
				import('$lib/utils/spreadsheet')
			]);
			excelWorkbook = await parseSpreadsheet(arrayBuffer, item.name);
