<script lang="ts">
	...
	import type ChatControlsType from './ChatControls.svelte';
	...
	let ChatControlsComponent: typeof ChatControlsType | null = null;
	let controlPaneComponent: InstanceType<typeof ChatControlsType> | undefined;

	const ensureChatControls = async () => {
		if (!ChatControlsComponent) {
			ChatControlsComponent = (await import('./ChatControls.svelte')).default;
		}
		await tick();
	};
	...
	const localizeErrorMessage = (error) => {
		const rawMessage = getErrorMessage(error);
		const message = rawMessage || (typeof error === 'object' ? '' : `${error}`);
	...
	const detachCurrentFailedResponse = () => {
		if (!history?.currentId) return false;

		const currentMessage = history.messages[history.currentId];
		if (!currentMessage?.error || currentMessage.content) return false;

		const parentId = currentMessage.parentId ?? null;
		if (parentId && history.messages[parentId]?.childrenIds) {
			history.messages[parentId].childrenIds = history.messages[parentId].childrenIds.filter(
				(id) => id !== currentMessage.id
			);
		}

		delete history.messages[currentMessage.id];
			...
			if (message) {
				if (type === 'status') {
					...
					if (toastType === 'success') {
						...
						toast.error(localizeErrorMessage(toastContent) || toastContent);
					...
					const error = 'Browser-side JavaScript execution is disabled for security.';
					console.warn(error);
					cb?.({ error });
				} else if (type === 'input') {
					eventCallback = cb;
	...
	const onMessageHandler = async (event: {
		...
		const isEmbeddingParent = window.parent !== window && event.source === window.parent;
		...
		const isTrusted =
			isSameOrigin || (isEmbeddingParent && ($settings?.iframeSandboxAllowSameOrigin ?? false));
	...
	onMount(() => {
		...
		audioQueueInstance.setOutputDevice($settings?.audio?.outputDeviceId);
	...
	const initNewChat = async () => {
		...
		if ($page.url.searchParams.get('image-generation') === 'true') {
			imageGenerationEnabled.set(true);
		}
		...
		if ($desktopEvent) {
			...
			if (event.type === 'call') {
				await applyDesktopShortcutAction(event.data?.shortcutAction);
			...
			} else if (event.type === 'action:activate') {
				await applyDesktopShortcutAction(event.data?.action);
	...
	const loadChat = async (targetChatId = chatIdProp) => {
		...
		if (chat) {
			...
			if (chatContent) {
				...
				const chatModels = chatContent?.models;
				selectedModels = Array.isArray(chatModels) ? chatModels : [chatModels ?? ''];
				...
				const rawHistory = chatContent?.history;
				history =
				...
				if (history.currentId === undefined) {
	...
	const getAudioFormatFromDataUrl = (dataUrl: string) => {
		const contentType = dataUrl.split(';', 1)[0].replace('data:', '').toLowerCase();
		const audioFormat = contentType.split('/')[1] || 'wav';
		if (audioFormat === 'mpeg') {
			return 'mp3';
		}
		if (audioFormat === 'x-wav') {
			return 'wav';
		}
		return audioFormat;
	};
	...
	const sendMessageSocket = async (
		...
		files.push(...(userMessage?.files ?? []).filter((item) => isContextFile(item)));
		// Remove duplicates
			...
			messages = messages
				.map((message) => {
					...
					const multimodalFileParts = [
						...
						...audioFiles
							.filter((file) => file.url?.startsWith('data:audio/') && file.url.includes(','))
							.map((file) => ({
								type: 'input_audio',
								input_audio: {
									data: file.url.split(',', 2)[1],
									format: getAudioFormatFromDataUrl(file.url)
								}
							}))
					];
	...
	const handleOpenAIError = async (error, responseMessage) => {
		...
		errorMessage =
			localizeErrorMessage(innerError) || $i18n.t(`Uh-oh! There was an issue with the response.`);
		toast.error(errorMessage);
