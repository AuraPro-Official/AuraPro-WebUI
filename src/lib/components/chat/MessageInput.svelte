<script lang="ts">
	...
	import AudioFileItem from '../common/AudioFileItem.svelte';
	...
	import Boindex from '../icons/Boindex.svelte';
	...
	const uploadFileHandler = async (file, process = true, itemData = {}) => {
		...
		if (!$temporaryChatEnabled) {
			...
			if (process === false && file.type.startsWith('audio/')) {
					...
					<div class={recording ? '' : 'hidden'}>
						<VoiceRecording
							...
							onConfirm={async (data) => {
								...
								if (data?.file) {
									// Attach the recording as a playable voice-message chip so the
									// user can review it and add text before sending. Only submit
									// immediately when the user opted into auto-send.
									const uploadedFile = await uploadFileHandler(data.file, false, {
										type: 'audio',
										content_type: data.file.type
									});
									if (uploadedFile && ($settings?.speechAutoSend ?? false)) {
										await tick();
									...
									<div class="flex flex-1 items-center min-w-0 overflow-x-auto scrollbar-none">
										...
										{#if $ragTranslationModeEnabled}
											<Tooltip content={$i18n.t('RAG Translation Mode')} placement="top">
												...
												>
													<Boindex className="size-3.5" strokeWidth="2" />
												...
												<button
													...
													on:click={async () => {
														try {
															...
															if (stream) {
																recording = true;
																toast.info($i18n.t('Recording while AI is responding...'));
											...
											<div class=" flex items-center">
												...
												<Tooltip content={$i18n.t('Voice mode')}>
													<button
														...
														type="button"
														on:click={async () => {
															const sttEngine = getEffectiveSttEngine();
															if (sttEngine === 'multimodal') {
																try {
																	let stream = await navigator.mediaDevices.getUserMedia({
																		audio: getAudioInputConstraint()
																	});
