<script lang="ts">
	...
	const getVoiceTranslationMode = () => $settings?.audio?.simultaneous?.mode ?? 'off';
	const isVoiceTranslationModeEnabled = () =>
		['face_to_face', 'call'].includes(getVoiceTranslationMode());

	const getChineseRatio = (content = '') => {
		const compact = String(content).replace(/\s+/g, '');
		if (!compact) return 0;
		const chinese = compact.match(/[\u4e00-\u9fff]/g)?.length ?? 0;
		return chinese / compact.length;
	};

	const shouldSpeakAssistantContent = (content = '') => {
		const mode = getVoiceTranslationMode();
		if (mode === 'off') return false;
		if (mode === 'face_to_face') return true;
		if (mode === 'call') {
			// In call interpretation, Chinese output is meant for the local user to read.
			// Foreign-language output is spoken to the other person or the virtual call device.
			return getChineseRatio(content) < 0.35;
		}
		return false;
	};
	...
	const isVirtualOutputSelected = () => {
		const label = getSelectedOutputDevice()?.label?.toLowerCase() ?? '';
		return Boolean(label && virtualOutputKeywords.some((keyword) => label.includes(keyword)));
	};

	const ensureInterpretationModeForVoiceTranslation = async () => {
		if (!isVoiceTranslationModeEnabled()) return;
		interpretationModeEnabled.set(true);
		translationModeEnabled.set(false);
		learningModeEnabled.set(false);
		manuscriptTranslationModeEnabled.set(false);
		await tick();
	};
	...
	const chatEventHandler = async (e) => {
		...
		if (currentMessageId === id) {
			...
			try {
				if (!shouldSpeakAssistantContent(content)) {
				...
				<div class="flex items-center justify-between gap-3">
					<div class="min-w-0">
						<div class="font-medium">
							{getVoiceTranslationMode() === 'call'
								? $i18n.t('Call Interpretation Mode')
								: $i18n.t('Face-to-face Interpretation Mode')}
						</div>
