<script lang="ts">
	...
	const saveContextCompactionSettings = () => {
		saveSettings({
			contextCompaction: {
	...
	onMount(async () => {
		...
		contextCompactionEnabled = $settings?.contextCompaction?.enabled ?? true;
...
>
	...
	<div class=" space-y-3 overflow-y-scroll max-h-[28rem] md:max-h-full">
		<div>
			...
			<div>
				<div class="py-0.5 flex w-full justify-between">
					<div id="context-compaction-label" class="self-center text-xs">
						{$i18n.t('Context Compaction')}
					</div>

					<button
						aria-labelledby="context-compaction-label context-compaction-mode"
					...
					>
						<span class="ml-2 self-center" id="context-compaction-mode">
							{contextCompactionEnabled ? $i18n.t('Default') : $i18n.t('Off')}
						</span>
					</button>
				</div>
			</div>
