			...
			<div>
				<div class="pr-1.5">
					<div class="flex items-center justify-between gap-4">
						<div>
							<div class="font-medium">{$i18n.t('Default extension mode')}</div>
							<div class="mt-0.5 text-xs text-gray-500">
								{$i18n.t('Automatically enable this extension mode when Open WebUI starts.')}
							</div>
						</div>
						<select
							class="min-w-44 rounded-lg border border-gray-100 bg-gray-50 px-3 py-2 text-right text-xs outline-hidden dark:border-gray-800 dark:bg-gray-900"
							bind:value={defaultExtensionMode}
							aria-label={$i18n.t('Default extension mode')}
						>
							<option value="">{$i18n.t('None')}</option>
							<option value="translation">{$i18n.t('Translation Mode')}</option>
							<option value="manuscript_translation"
								>{$i18n.t('Manuscript Translation Mode')}</option
							>
							<option value="interpretation">{$i18n.t('Simultaneous Interpretation')}</option>
							<option value="learning">{$i18n.t('Learning Mode')}</option>
							<option value="rag_translation">{$i18n.t('RAG Translation Mode')}</option>
						</select>
					</div>
				</div>
