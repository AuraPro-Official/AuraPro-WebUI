		...
		setTimeout(async () => {
			...
			if (selection.toString().trim().length > 0) {
				...
				if (buttonsContainerElement) {
					...
					buttonsContainerElement.style.position = 'fixed';
					buttonsContainerElement.style.zIndex = '2147483647';

					const margin = 8;
					const estimatedWidth = $mobile ? 260 : 360;
					const left = Math.min(
					...
					buttonsContainerElement.style.top = `${Math.min(rect.bottom + 5, window.innerHeight - 80)}px`;
				}
			} else {
				closeFloatingButtons();
			}
