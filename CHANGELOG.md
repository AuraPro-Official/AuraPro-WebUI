...
- 知识库普通文档、双语文本和 EPUB 导入新增实时阶段、进度、耗时及预计剩余时间展示。
- 首次加载本地嵌入和词语对齐模型时显示长时间等待提示。

### Changed

- 知识库文件处理状态会持续保存，刷新页面后仍可恢复当前进度。
- 补充知识库新增界面的简体中文、繁体中文、英文和西班牙语文案。

### Fixed

- 修复首次发送消息时偶发需要刷新后才显示回复的问题。
- 修复流式回复可能在生成完成后才一次性显示的问题。
- 修复双语或 EPUB 导入失败后界面仍按成功流程关闭的问题。
...
- 新增测试版本索引模式（用于翻译和找文献资料），具体更新日志将在 v3.7.0 正式发布时详细标注。
...
- 新增 `medium_Q4` 模型，面向 Mac 16G 和 16G 显存显卡设计。
