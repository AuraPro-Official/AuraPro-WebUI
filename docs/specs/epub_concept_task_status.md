...
- **Next action:** Continue T-090 release acceptance: verify packaged Desktop/WebUI version compatibility and repeat the Desktop-managed path on the supported OS/architecture matrix. The real local-book E2E is recorded above; do not send the book to a cloud Batch provider until an administrator explicitly configures and authorizes that offline run.
