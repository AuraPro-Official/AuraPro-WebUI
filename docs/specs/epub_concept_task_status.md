...
- **Current focus:** T-050 — complete review of local-only inference adapters and derived vector-index boundary before implementing search orchestration in T-060.
- **Current implementation state:** Local/private endpoint validation rejects public endpoints unless an administrator explicitly trusts a private DNS hostname. Inference unavailability is represented as a degraded result; it never falls back to a cloud adapter. The vector boundary checks the source substring and hash before backend upsert.
- **Next action:** Review the T-050 implementation and integrate its protocols into the T-060 search pipeline; do not add a cloud fallback.
