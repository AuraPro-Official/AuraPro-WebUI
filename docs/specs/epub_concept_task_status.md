...
- **Next action:** Replace the EPUB module's standalone private-model HTTP adapters with AuraPro's server-owned RAG embedding/reranking and llama.cpp model dispatch, then validate a configured local server can import, index, and browse a textual EPUB through the full application path.
