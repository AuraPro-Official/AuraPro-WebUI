...
| T-095 | Provision a healthy native SQLite extension test runtime | in_progress | T-025 | Required before accepting the persistent `sqlite-vec` backend. The Homebrew Python 3.14 `pyexpat` extension cannot load (`_XML_SetAllocTrackerActivationThreshold` missing), so pip cannot install the locked `sqlite-vec==0.1.9`; the temporary Python 3.12 test build runs acceptance tests but lacks `sqlite3.Connection.enable_load_extension`. Do not claim native vector persistence until a runtime with both capabilities is verified. |
