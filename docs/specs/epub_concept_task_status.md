...
| T-090 | End-to-end, migration, resilience, and operational validation | in_progress | T-020, T-040, T-060, T-070, T-080 | Component acceptance suite passes: parser 5, store 6, Batch 10, local inference 16, search 5, API 5, frontend client 3. Local production startup now constructs an independent persistent SQLite service, rejects remote PostgreSQL misconfiguration without falling back to the main database, and exposes a credential-free runtime health snapshot (`test_epub_runtime.py` — 5 tests passed; sqlite-vec suite — 4 tests passed). PostgreSQL/pgvector parity, real configured local server validation, and resilience scenarios remain. |
...
| T-110 | Desktop-managed local runtime packaging | pending | T-050, T-090, D-010 | After the Homebrew local E2E baseline passes, replace end-user static `EPUB_CONCEPT_LOCAL_LLM_*` configuration with AuraPro Desktop-owned runtime/model lifecycle integration. Acceptance: install Desktop + WebUI only; first-run local model download/progress/recovery; dynamic loopback endpoint; no Homebrew/Python/manual model-server setup. Remote shared-server deployment remains administrator-owned. |
...
- **Next action:** Complete the Homebrew llama.cpp local E2E baseline, then execute T-110 to validate the Desktop-managed path with no external end-user installation.
