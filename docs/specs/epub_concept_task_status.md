...
| T-050 | Local-only inference adapters and derived vector index | in_progress | T-030 | Local/private endpoint policy, embedding/reranker/concept-resolver interfaces, explicit availability/degraded behavior, and exact parent-window vector binding are implemented. Python 3.12: `test_epub_local_inference.py` — 8 tests passed. Search integration is complete; production native SQLite/pgvector backends and server-owned adapter configuration remain. |
...
| T-070 | Authenticated REST API and admin authorization | completed | T-040, T-060 | Python 3.12 `test_epub_api.py` — 4 tests passed: authenticated shared reads/search, admin-only mutations, and fail-closed missing startup service. The test isolates only the auth dependency identity, while exercising the real FastAPI router and service. |
...
| T-090 | End-to-end, migration, resilience, and operational validation | in_progress | T-020, T-040, T-060, T-070, T-080 | Component acceptance suite passes: parser 5, store 6, Batch 10, local inference 8, search 5, API 4, frontend client 3. Local production startup now constructs an independent persistent SQLite service and rejects remote PostgreSQL misconfiguration without falling back to the main database (`test_epub_runtime.py` — 3 tests passed). PostgreSQL/pgvector parity and resilience scenarios remain. |
...
| Authenticated API (T-070) | `78854f3` | pending | FastAPI authorization/service suite 4/4 passes. Push remains pending repository administrator approval. |
...
- **Current implementation state:** The frontend keeps reads on authenticated `/epub` routes and renders complete passages plus explicit exact excerpts. The administrator route is separate, presents imports, Batch controls, concept review, and derived-index operations, and sends no provider credentials. Backend authorization remains authoritative. Local startup now attaches a persistent independent SQLite service; native derived-vector persistence and the remote PostgreSQL implementation remain.
- **Next action:** Validate a configured local server can import and browse a textual EPUB through the full application startup path, then continue the PostgreSQL/pgvector migration design without using the main WebUI database as canonical storage.
