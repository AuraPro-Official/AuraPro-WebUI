...
| T-110 | Desktop-managed local runtime packaging | pending | T-050, T-090, D-010 | The Homebrew baseline and real local-book E2E passed on 2026-08-02. Replace end-user static `EPUB_CONCEPT_LOCAL_LLM_*` configuration with AuraPro Desktop-owned runtime/model lifecycle integration, including locked `sqlite-vec==0.1.9` in Desktop's managed Python environment. Acceptance: install Desktop + WebUI only; first-run local model download/progress/recovery; dynamic loopback endpoint; no Homebrew/Python/manual model-server setup. Remote shared-server deployment remains administrator-owned. |
