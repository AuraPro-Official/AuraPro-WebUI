# AuraPro WebUI Security Notice

AuraPro WebUI is an AuraPro distribution/fork derived from Open WebUI. For AuraPro-specific vulnerabilities or packaging issues, use the AuraPro WebUI repository security channels or issue tracker. Upstream Open WebUI security guidance below is retained for upstream-origin code and architecture.
