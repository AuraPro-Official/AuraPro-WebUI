...
def print_report(report: dict[str, Any]) -> None:
    ...
    print()
    print(c(f"job {report['job_id']}  state={report['state']}  "
            f"failed items={report['failed_item_count']}", BOLD))
    if report.get("results_pending_retrieval"):
        print(c("  provider output could not be retrieved; nothing was measured "
                "and nothing was recorded. Re-run.", RED, BOLD))
        return
    ...
    for packet in sorted(packets, key=lambda item: (_packet_class(item), item["custom_id"])):
        ...
        if not packet.get("grounded"):
            print(f"  {label:<14}{_packet_class(packet):<20}"
    ...
    print()
    for failure_class in (*REPORTED_CLASSES, *sorted(
        {_packet_class(packet) for packet in packets} - set(REPORTED_CLASSES)
    )):
        ...
        totals = {
            name: sum(_cell(packet, key, name) for packet in measured)
            for name, key in COLUMNS
        }
        colour = GREEN if failure_class == "EVIDENCE_AMBIGUOUS" else YELLOW
        print(c(f"  {failure_class}  ({len(group)} packets, {len(measured)} measurable)",
                colour, BOLD))
...
def parse_args(argv: Sequence[str]) -> argparse.Namespace:
    ...
    parser.add_argument("--snapshot", default=None,
                        help="where to keep the VACUUM INTO copy (default: a temporary file)")
    parser.add_argument("--json", action="store_true",
                        help="emit the raw content-free report instead of the table")
...
def main(argv: Sequence[str]) -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="epub-dry-run-") as workspace:
        snapshot_path = (
            ...
            if args.snapshot
            else Path(workspace) / "snapshot.db"
        )
    ...
    print(c(f"  write check: {len(before)} tables, "
            ...
            GREEN if not changed else RED, BOLD))
    print(c(f"  batch_items rows {after['batch_items']['rows']} "
            f"checksum {after['batch_items']['checksum']}", DIM))
